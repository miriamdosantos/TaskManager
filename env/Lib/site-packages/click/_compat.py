    ...
    def __init__(
        ...
        **extra: t.Any,
        ...
        self._stream = stream = t.cast(
            t.BinaryIO,
            _FixupStream(stream, force_readable, force_writable),
        )
...
def open_stream(
    ...
    if os.fsdecode(filename) == "-":
        if any(m in mode for m in ["w", "a", "x"]):
            ...
            return (
                get_text_stdout(encoding=encoding, errors=errors),
                False,
            )
        if binary:
    ...
    while True:
        ...
        try:
            fd = os.open(
                tmp_filename, flags, 0o666 if perm is None else perm
            )
            break
        except OSError as e:
...
def _is_jupyter_kernel_output(stream: t.IO[t.Any]) -> bool:
    while isinstance(
        stream, (_FixupStream, _NonClosingTextIOWrapper)
    ):
...
def should_strip_ansi(
    ...
    if color is None:
        if stream is None:
            stream = sys.stdin
        return not isatty(stream) and not _is_jupyter_kernel_output(
            stream
        )
    return not color
