...
def from_bytes(
    ...
    for encoding_iana in prioritized_encodings + IANA_SUPPORTED:
        ...
        strip_sig_or_bom: bool = (
            ...
            and should_strip_sig_or_bom(encoding_iana)
        )

        if (
            encoding_iana in {"utf_16", "utf_32"}
        ...
        try:
            is_multi_byte_decoder: bool = is_multi_byte_encoding(
                encoding_iana
            )
        ...
        try:
            if (
                ...
                and is_multi_byte_decoder is False
            ):
                str(
                    (
                        sequences[: int(50e4)]
        ...
        if not is_multi_byte_decoder:
            target_languages: List[str] = encoding_languages(
                encoding_iana
            )
        else:
        ...
        if (
            ...
            and (
                specified_encoding is None
                or specified_encoding in tested
            )
            and "ascii" in tested
            and "utf_8" in tested
        ):
    ...
    if len(results) == 0:
        ...
        if fallback_specified:
            ...
            or (
                fallback_u8
                and fallback_ascii
                and fallback_u8.fingerprint
                != fallback_ascii.fingerprint
            )
