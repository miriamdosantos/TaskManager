...
def alphabet_languages(
    ...
    for language, language_characters in FREQUENCIES.items():
        target_have_accents, target_pure_latin = get_target_features(
            language
        )
...
def characters_popularity_compare(
    ...
    for character, character_rank in zip(
        ...
        if (
            ...
            and abs(
                character_rank_projection - character_rank_in_language
            )
            > 4
        ):
            continue
        ...
        if (
            len(characters_before_source) == 0
            and before_match_count <= 4
        ):
            character_approved_count += 1
            continue

        if (
            len(characters_after_source) == 0
            and after_match_count <= 4
        ):
            character_approved_count += 1
            continue
    ...
    if any(len(index_results[e]) > 1 for e in index_results):
        ...
        for language in index_results:
            filtered_results.append(
                (language, max(index_results[language]))
            )
