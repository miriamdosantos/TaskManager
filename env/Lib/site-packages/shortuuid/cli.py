...
def cli(*args: Any) -> None:
    ...
    encode_parser = subparsers.add_parser(
        "encode",
        help="Encode a UUID into a short UUID",
        description=encode.__doc__,
    )
    encode_parser.add_argument(
        "uuid", type=UUID, help="UUID to be encoded"
    )
    encode_parser.set_defaults(func=encode_cli)

    decode_parser = subparsers.add_parser(
        "decode",
        help="Decode a short UUID into a UUID",
        description=decode.__doc__,
    )
    decode_parser.add_argument(
        "shortuuid", type=str, help="Short UUID to be decoded"
    )
