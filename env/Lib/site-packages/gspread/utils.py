...
def convert_credentials(credentials: Credentials) -> Credentials:
    ...
    if (
        "oauth2client" in module
        and cls == "ServiceAccountCredentials"
    ):
...
def a1_range_to_grid_range(
    ...
    start_row_index, start_column_index = _a1_to_rowcol_unbounded(
        start_label
    )
...
def column_letter_to_index(column: str) -> int:
    ...
    try:
        ...
        raise InvalidInputValue(
            "invalid value: {}, must be a column letter".format(
                column
            )
        )
...
def rightpad(
    row: List[Any], max_len: int, padding_value: Any = ""
) -> List[Any]:
...
def fill_gaps(
    ...
    try:
        max_cols = (
            max(len(row) for row in L) if cols is None else cols
        )
        ...
        return [
            rightpad(row, max_cols, padding_value=padding_value)
            for row in L
        ]
    except ValueError:
        return [[]]
...
def cell_list_to_rect(
    ...
    all_row_keys = chain.from_iterable(
        row.keys() for row in rows.values()
    )
...
def combined_merge_values(
    ...
    for merge in merges:
        ...
        if (
            merge_start_row > max_row_index
            or merge_start_col > max_col_index
        ):
            continue
...
def convert_hex_to_colors_dict(hex_color: str) -> Mapping[str, float]:
    ...
    if len(hex_color) != 6:
        raise ValueError(
            "Hex color code must be in the format '#RRGGBB'."
        )
    ...
    try:
        ...
        raise ValueError(
            f"Invalid character in hex color string: #{hex_color}"
        ) from ex
