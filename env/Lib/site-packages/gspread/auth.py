...
def get_config_dir(
    config_dir_name: str = "gspread",
    os_is_windows: bool = os.name == "nt",
) -> Path:
...
DEFAULT_AUTHORIZED_USER_FILENAME = (
...
def authorize(
    ...
    return Client(
        auth=credentials, session=session, http_client=http_client
    )
...
def oauth(
    ...
    credentials_filename: Union[
        str, Path
    ] = DEFAULT_CREDENTIALS_FILENAME,
    authorized_user_filename: Union[
        str, Path
    ] = DEFAULT_AUTHORIZED_USER_FILENAME,
    http_client: HTTPClientType = HTTPClient,
) -> Client:
...
def api_key(
    token: str, http_client: HTTPClientType = HTTPClient
) -> Client:
