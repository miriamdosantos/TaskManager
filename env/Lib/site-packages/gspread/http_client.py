...
ParamsType = MutableMapping[
    str, Optional[Union[str, int, bool, float, List[str]]]
]
...
FileType = Optional[
    Union[
        ...
        MutableMapping[
            str, Tuple[str, IO[Any], str, MutableMapping[str, str]]
        ],
    ]
]
...
class HTTPClient:
    ...
    def __init__(
        self, auth: Credentials, session: Optional[Session] = None
    ) -> None:
    ...
    def batch_update(
        ...
        """
        r = self.request(
            "post", SPREADSHEET_BATCH_UPDATE_URL % id, json=body
        )
    ...
    def export(
        self, file_id: str, format: str = ExportFormat.PDF
    ) -> bytes:
    ...
    def insert_permission(
        ...
        url = "{}/{}/permissions".format(
            DRIVE_FILES_API_V3_URL, file_id
        )
        payload = {
            "type": perm_type,
            "role": role,
        ...
        if perm_type == "domain":
            ...
            raise ValueError(
                "Invalid permission type: {}".format(perm_type)
            )
    ...
    def import_csv(
        self, file_id: str, data: Union[str, bytes]
    ) -> Any:
