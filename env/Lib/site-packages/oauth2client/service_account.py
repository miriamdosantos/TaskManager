    ...
    * ``.p12`` key (stores PKCS12 key and certificate)
    ...
    def _from_p12_keyfile_contents(
        ...
        signer = crypt.Signer.from_string(
            private_key_pkcs12, private_key_password
        )
        credentials = cls(
            service_account_email,
            signer,
    ...
    def _generate_assertion(self):
        ...
        return crypt.make_signed_jwt(
            self._signer, payload, key_id=self._private_key_id
        )
    ...
    def from_json(cls, json_data):
        ...
        if pkcs12_val is None:
            private_key_pkcs8_pem = json_data[
...
class _JWTAccessCredentials(ServiceAccountCredentials):
    ...
    def get_access_token(self, http=None, additional_claims=None):
        ...
        if additional_claims is None:
            ...
            token, unused_expiry = self._create_token(
                additional_claims
            )
            return client.AccessTokenInfo(
                access_token=token,
    ...
    def _create_token(self, additional_claims=None):
        ...
        lifetime = datetime.timedelta(
            seconds=self._MAX_TOKEN_LIFETIME_SECS
        )
        expiry = now + lifetime
        payload = {
