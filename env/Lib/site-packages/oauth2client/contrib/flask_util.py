...
try:
    ...
    raise ImportError(
        "The flask utilities require flask 0.9 or newer."
    )
...
class UserOAuth2(object):
        ...
        **kwargs
        ...
        if scopes is None:
            scopes = app.config.get(
                "GOOGLE_OAUTH2_SCOPES", _DEFAULT_SCOPES
            )
        self.scopes = scopes

        self._load_config(
            client_secrets_file, client_id, client_secret
        )

        app.register_blueprint(self._create_blueprint())

    def _load_config(
        self, client_secrets_file, client_id, client_secret
    ):
    ...
    def _create_blueprint(self):
        ...
        bp.add_url_rule(
            "/oauth2authorize", "authorize", self.authorize_view
        )
        bp.add_url_rule(
            "/oauth2callback", "callback", self.callback_view
        )
    ...
    def authorize_url(self, return_url, **kwargs):
        ...
        """
        return url_for(
            "oauth2.authorize", return_url=return_url, **kwargs
        )

    def required(
        self, decorated_function=None, scopes=None, **decorator_kwargs
    ):
        ...
        def curry_wrapper(wrapped_function):
            ...
            def required_wrapper(*args, **kwargs):
                return_url = decorator_kwargs.pop(
                ...
                if (
                    ...
                    and self.credentials.has_scopes(requested_scopes)
                ):
