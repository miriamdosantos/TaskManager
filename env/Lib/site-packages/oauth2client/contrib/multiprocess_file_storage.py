...
def _load_credentials_file(credentials_file):
    ...
    for key, encoded_credential in iteritems(
        ...
        try:
            ...
            logger.warning(
                "Invalid credential {0} in file, ignoring.".format(
                    key
                )
            )
...
class _MultiprocessStorageBackend(object):
    ...
    def _write_credentials(self):
        if self._read_only:
            logger.debug(
                "In read-only mode, not writing credentials."
            )
            return

        _write_credentials_file(self._file, self._credentials)
        logger.debug(
            "Wrote credential file {0}.".format(self._filename)
        )

    def acquire_lock(self):
        self._thread_lock.acquire()
        locked = self._process_lock.acquire(
            timeout=INTERPROCESS_LOCK_DEADLINE
        )
...
def _get_backend(filename):
    ...
    with _backends_lock:
        if filename not in _backends:
            _backends[filename] = _MultiprocessStorageBackend(
                filename
            )
