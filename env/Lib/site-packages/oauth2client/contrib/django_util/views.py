...
def _get_flow_for_token(csrf_token, request):
    ...
    return (
        ...
        else jsonpickle.decode(flow_pickle)
    )
...
def oauth2_callback(request):
    ...
    signals.oauth2_authorized.send(
        sender=signals.oauth2_authorized,
        request=request,
...
def oauth2_authorize(request):
    ...
    scopes = request.GET.getlist(
        "scopes", django_util.oauth2_settings.scopes
    )
    ...
    if django_util.oauth2_settings.storage_model:
        ...
        # out user but we had credentials for it in the first place
            ...
            user_oauth = django_util.UserOAuth2(
                request, scopes, return_url
            )
            if user_oauth.has_credentials():
                return redirect(return_url)

    flow = _make_flow(
        request=request, scopes=scopes, return_url=return_url
    )
