...
def _get_oauth2_client_id_and_secret(settings_instance):
    ...
    """
    secret_json = getattr(
        settings_instance, "GOOGLE_OAUTH2_CLIENT_SECRETS_JSON", None
    )
    if secret_json is not None:
        return _load_client_secrets(secret_json)
    else:
        client_id = getattr(
            settings_instance, "GOOGLE_OAUTH2_CLIENT_ID", None
        )
        client_secret = getattr(
            settings_instance, "GOOGLE_OAUTH2_CLIENT_SECRET", None
        )
...
class OAuth2Settings(object):
    ...
    def __init__(self, settings_instance):
        ...
        middleware_settings = getattr(
            settings_instance, "MIDDLEWARE", None
        )
...
def _credentials_from_request(request):
    ...
    if (
        oauth2_settings.storage_model is None
...
class UserOAuth2(object):
    ...
    def get_authorize_redirect(self):
        ...
        return _redirect_with_params(
            "google_oauth:authorize", **get_params
        )
