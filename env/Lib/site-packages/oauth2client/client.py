...
_GCE_METADATA_URI = "http://" + os.getenv(
...
class Credentials(object):
    ...
    def new_from_json(cls, json_data):
        ...
        module_obj = __import__(
            module_name, fromlist=module_name.split(".")[:-1]
        )
...
class OAuth2Credentials(Credentials):
    ...
    def _do_retrieve_scopes(self, http, token):
        ...
        if resp.status == http_client.OK:
            d = json.loads(content)
            self.scopes = set(
                _helpers.string_to_scopes(d.get("scope", ""))
            )
        else:
            error_msg = "Invalid response {0}.".format(resp.status)
            try:
...
class AccessTokenCredentials(OAuth2Credentials):
    ...
    def from_json(cls, json_data):
        ...
        retval = AccessTokenCredentials(
            data["access_token"], data["user_agent"]
        )
        return retval
...
def _detect_gce_environment():
    ...
    try:
        ...
        return (
            ...
            and response.get(_METADATA_FLAVOR_HEADER)
            == _DESIRED_METADATA_FLAVOR
        )
    except (
        socket.error
    ):  # socket.timeout or socket.error(64, 'Host is down')
        logger.info(
            "Timeout attempting to reach GCE metadata service."
        )
        return False
...
class GoogleCredentials(OAuth2Credentials):
    ...
    def from_json(cls, json_data):
        ...
        if (
            ...
            return (
                service_account.ServiceAccountCredentials.from_json(
                    data
                )
            )
        elif (
            ...
            return service_account._JWTAccessCredentials.from_json(
                data
            )
    ...
    def _implicit_credentials_from_files():
        ...
        try:
            ...
            _raise_exception_for_reading_json(
                credentials_filename, extra_help, error
            )
    ...
    def from_stream(credential_filename):
        ...
        Raises:
            ApplicationDefaultCredentialsError: raised when the credentials
                                                fail to be retrieved.
        """
        if credential_filename and os.path.isfile(
            credential_filename
        ):
            try:
                return _get_application_default_credential_from_file(
...
def _save_private_file(filename, json_contents):
    ...
    file_desc = os.open(
        temp_filename, os.O_WRONLY | os.O_CREAT, 0o600
    )
...
def _get_well_known_file():
    ...
    return os.path.join(
        default_config_dir, _WELL_KNOWN_CREDENTIALS_FILE
    )
...
def _raise_exception_for_reading_json(
...
def verify_id_token(
    ...
    if resp.status == http_client.OK:
        ...
        return crypt.verify_signed_jwt_with_certs(
            id_token, certs, audience
        )
    else:
        raise VerifyJwtTokenError(
            "Status code: {0}".format(resp.status)
        )
...
class OAuth2WebServerFlow(Flow):
    ...
    def step1_get_authorize_url(self, redirect_uri=None, state=None):
        ...
        if self.redirect_uri is None:
            raise ValueError(
                "The value of redirect_uri must not be None."
            )
    ...
    def step1_get_device_and_user_codes(self, http=None):
        ...
        if self.device_uri is None:
            raise ValueError(
                "The value of device_uri must not be None."
            )
    ...
    def step2_exchange(
        ...
        if code is not None and device_flow_info is not None:
            raise ValueError(
                "Cannot provide both code and device_flow_info."
            )

        if code is None:
            code = device_flow_info.device_code
        elif not isinstance(
            code, (six.string_types, six.binary_type)
        ):
            if "code" not in code:
        ...
        if resp.status == http_client.OK and "access_token" in d:
            ...
            if "error" in d:
                ...
                error_msg = str(d["error"]) + str(
                    d.get("error_description", "")
                )
            else:
