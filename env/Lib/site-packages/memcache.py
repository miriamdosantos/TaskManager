...
class Client(local):
    ...
    def get_stats(self, stat_args=None):
        ...
        for s in self.servers:
            ...
            while 1:
                ...
                serverData[stats[1].encode("ascii")] = stats[
                    2
                ].encode("ascii")
    ...
    def delete(self, key, time=0):
        ...
        try:
            ...
            if line and line.strip() in [b"DELETED", b"NOT_FOUND"]:
                return 1
            self.debuglog(
                "Delete expected DELETED or NOT_FOUND, got: %s"
                % repr(line)
            )
        except socket.error as msg:
            if isinstance(msg, tuple):
    ...
    def _map_and_prefix_keys(self, key_iterable, key_prefix):
        ...
        for orig_key in key_iterable:
            if isinstance(orig_key, tuple):
                ...
                server, key = self._get_server(
                    key_prefix + str_orig_key
                )

            # Now check to make sure key length is proper ...
            if self.do_check_key:
                self.check_key(
                    str_orig_key, key_extra_len=key_extra_len
                )
    ...
    def set_multi(
        self, mapping, time=0, key_prefix="", min_compress_len=0
    ):
        """
        ...
        if type(val) == str:
            ...
            if self.picklerIsKeyword:
                pickler = self.pickler(
                    file, protocol=self.pickleProtocol
                )
            else:
    ...
    def _cmd_builder(self, cmd, key, time, store_info):
        ...
        if isinstance(store_info[2], bytes):
            ...
            raise _Error(
                "_cmd_builder: unknown data type (%s)"
                % type(store_info[2])
            )
        ...
        def _unsafe_set():
            ...
            if cmd == "cas":
                if key not in self.cas_ids:
                    return self._set(
                        "set", key, val, time, min_compress_len
                    )
                fullcmd = self._cmd_builder(
                    cmd, key, time, store_info
                )
            else:
                fullcmd = self._cmd_builder(
                    cmd, key, time, store_info
            ...
            try:
                ...
                return (
                    server.expect(b"STORED", raise_exception=True)
                    == b"STORED"
                )
            except socket.error as msg:
                if isinstance(msg, tuple):
    ...
    def _get(self, cmd, key):
        ...
        def _unsafe_get():
            ...
            try:
                server.send_cmd(
                    "{0} {1}".format(cmd, key).encode("utf-8")
                )
            ...
            try:
                ...
                while line and line != b"END":
                    ...
                    if rkey is not None:
                        ...
                        retvals[prefixed_to_orig_key[rkey]] = (
                            val  # un-prefix returned key.
                        )
                    line = server.readline()
            except (_Error, socket.error) as msg:
                if isinstance(msg, tuple):
    ...
    def _expect_cas_value(
        self, server, line=None, raise_exception=False
    ):
        if not line:
    ...
    def check_key(self, key, key_extra_len=0):
        ...
        if (
            ...
            and keylen + key_extra_len > self.server_max_key_length
        ):
...
class _Host(object):
    ...
    def __init__(
        ...
        if not m:
            raise ValueError(
                'Unable to parse connection string: "%s"' % host
            )
    ...
    def mark_dead(self, reason):
        self.debuglog(
            "MemCache: %s: %s.  Marking dead." % (self, reason)
        )
...
if __name__ == "__main__":
    ...
    if "--do-unix" in sys.argv:
        serverList.append(
            [os.path.join(os.getcwd(), "memcached.socket")]
        )
    ...
    for servers in serverList:
        ...
        print(
            "Testing set_multi() with no memcacheds running", end=" "
        )
        mc.disconnect_all()
