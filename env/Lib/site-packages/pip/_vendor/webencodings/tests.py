...
def test_invalid_label():
    assert_raises(
        LookupError, decode, b"\xEF\xBB\xBF\xc3\xa9", "invalid"
    )
...
def test_decode():
    ...
    assert decode(b"\x00\xe9", "UTF-16BE") == (
        "é",
        lookup("utf-16be"),
    )
    assert decode(b"\xe9\x00", "UTF-16LE") == (
        "é",
        lookup("utf-16le"),
    )
...
def test_iter_decode():
    ...
    assert (
        iter_decode_to_string([b"\xEF\xBB\xBF\xc3\xa9"], "latin1")
        == "é"
    )
    assert (
    ...
    assert (
        iter_decode_to_string([b"\xFE\xFF\x00\xe9"], "latin1") == "é"
    )
    assert (
        iter_decode_to_string([b"\xFF\xFE\xe9\x00"], "latin1") == "é"
    )
    assert (
        iter_decode_to_string(
...
def test_iter_encode():
    ...
    assert (
        b"".join(iter_encode(["", "é", "", ""], "latin1")) == b"\xe9"
    )
    assert (
        b"".join(iter_encode(["", "é", "", ""], "utf-16"))
        == b"\xe9\x00"
    )
    assert (
        b"".join(iter_encode(["", "é", "", ""], "utf-16le"))
        == b"\xe9\x00"
    )
    assert (
        b"".join(iter_encode(["", "é", "", ""], "utf-16be"))
        == b"\x00\xe9"
    )
    assert (
        b"".join(
