...
ASCII_CTRL = frozenset(chr(i) for i in range(32)) | frozenset(
    chr(127)
)
...
def load(
    __fp: BinaryIO, *, parse_float: ParseFloat = float
) -> dict[str, Any]:
...
class Flags:
    ...
    def set(
        ...
        cont[key_stem][
            "recursive_flags" if recursive else "flags"
        ].add(flag)
    ...
    def is_(self, key: Key, flag: int) -> bool:
        ...
        if key_stem in cont:
            ...
            return (
                flag in cont["flags"]
                or flag in cont["recursive_flags"]
            )
        return False
...
def parse_hex_char(
    ...
    if len(hex_str) != hex_len or not HEXDIGIT_CHARS.issuperset(
        hex_str
    ):
...
def parse_basic_str(
    ...
    while True:
        ...
        if char in error_on:
            raise suffixed_err(
                src, pos, f"Illegal character {char!r}"
            )
        pos += 1
...
def parse_value(  # noqa: C901
    ...
    if datetime_match:
        try:
            ...
            raise suffixed_err(
                src, pos, "Invalid date or datetime"
            ) from e
        return datetime_match.end(), datetime_obj
    localtime_match = RE_LOCALTIME.match(src, pos)
    if localtime_match:
        return localtime_match.end(), match_to_localtime(
            localtime_match
        )
    ...
    if number_match:
        return number_match.end(), match_to_number(
