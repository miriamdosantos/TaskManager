...
class Segment(NamedTuple):
    ...
    def adjust_line_length(
        ...
        if line_length < length:
            ...
            for segment in line:
                ...
                if (
                    line_length + segment_length < length
                    or segment.control
                ):
                    append(segment)
    ...
    def get_shape(
        ...
        max_width = (
            max(get_line_length(line) for line in lines)
            if lines
            else 0
        )
    ...
    def set_shape(
        ...
        shaped_lines[:] = [
            adjust_line_length(line, width, style=style)
            for line in lines
        ]
        if len(shaped_lines) < _height:
            shaped_lines.extend(
                [blank] * (_height - len(shaped_lines))
            )
        return shaped_lines
    ...
    def align_middle(
        ...
        lines = (
            [[blank]] * top_lines + lines + [[blank]] * bottom_lines
        )
        return lines
    ...
    def simplify(
        ...
        for segment in iter_segments:
            if (
                last_segment.style == segment.style
    ...
    def strip_links(
        ...
        for segment in segments:
            if segment.control or segment.style is None:
                ...
                yield cls(
                    text, style.update_link(None) if style else None
                )
