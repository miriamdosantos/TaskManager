...
def _get_attr_fields(
    obj: Any,
) -> Sequence["_attr_module.Attribute[Any]"]:
...
def _is_dataclass_repr(obj: object) -> bool:
    ...
    try:
        return (
            obj.__repr__.__code__.co_filename == dataclasses.__file__
        )
...
def _ipy_display_hook(
    ...
    if console.is_jupyter:
        ...
        if is_object_one_of_types(
            value, JUPYTER_CLASSES_TO_NOT_RENDER
        ):
            return
...
class Node:
    ...
    def check_length(
        self, start_length: int, max_length: int
    ) -> bool:
        ...
        while line_no < len(lines):
            ...
            if line.expandable and not line.expanded:
                if expand_all or not line.check_length(max_width):
                    lines[line_no : line_no + 1] = line.expand(
                        indent_size
                    )
            line_no += 1
...
class _Line:
    ...
    def expand(self, indent_size: int) -> Iterable["_Line"]:
        ...
        if node.key_repr:
            ...
            new_line = yield _Line(
                text=node.open_brace, whitespace=whitespace
            )
    ...
    def _traverse(
        ...
        if rich_repr_result is not None:
            ...
            if reached_max_depth:
                ...
                pop_visited(obj_id)
        elif _is_namedtuple(obj) and _has_default_namedtuple_repr(
            obj
        ):
            if reached_max_depth:
