...
class Syntax(JupyterMixin):
    ...
    def __init__(
        ...
        *,
        ...
        self.background_style = (
            Style(bgcolor=background_color)
            if background_color
            else Style()
        )
    ...
    def _get_line_numbers_color(self, blend: float = 0.3) -> Color:
        ...
        if (
            ...
            or background_color.is_system_defined
        ):
            return Color.default()
        foreground_color = self._get_token_color(Token.Text)
        if (
            foreground_color is None
            or foreground_color.is_system_defined
        ):
    ...
    def _get_number_styles(
        ...
        if console.color_system in ("256", "truecolor"):
            ...
            highlight_number_style = Style.chain(
                ...
                self._theme.get_style_for_token(Token.Text),
                Style(
                    bold=True, color=self._get_line_numbers_color(0.9)
                ),
                self.background_style,
            )
        else:
            number_style = background_style + Style(dim=True)
            highlight_number_style = background_style + Style(
                dim=False
            )
    ...
    def __rich_measure__(
        ...
        return Measurement(
            self._numbers_column_width, options.max_width
        )
    ...
    def _process_code(self, code: str) -> Tuple[bool, str]:
        ...
        processed_code = (
            textwrap.dedent(processed_code)
            if self.dedent
            else processed_code
        )
...
def _get_code_index_for_syntax_position(
    ...
    if line_number > lines_count or len(newlines_offsets) < (
        line_number + 1
    ):
        return None  # `line_number` is out of range
    line_index = line_number - 1
    line_length = (
        newlines_offsets[line_index + 1]
        - newlines_offsets[line_index]
        - 1
    )
...
if __name__ == "__main__":  # pragma: no cover
    ...
    console = Console(
        force_terminal=args.force_color, width=args.width
    )
