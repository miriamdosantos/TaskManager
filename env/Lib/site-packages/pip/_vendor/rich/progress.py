...
class ProgressColumn(ABC):
    ...
    def __init__(self, table_column: Optional[Column] = None) -> None:
        ...
        self._renderable_cache: Dict[
            TaskID, Tuple[float, RenderableType]
        ] = {}
...
class SpinnerColumn(ProgressColumn):
    ...
    def set_spinner(
        ...
        self.spinner = Spinner(
            spinner_name, style=spinner_style, speed=speed
        )
...
class TextColumn(ProgressColumn):
    ...
    def __init__(
        ...
        super().__init__(
            table_column=table_column or Column(no_wrap=True)
        )
...
class BarColumn(ProgressColumn):
    ...
    def render(self, task: "Task") -> ProgressBar:
        ...
        return ProgressBar(
            total=(
                max(0, task.total) if task.total is not None else None
            ),
            completed=max(0, task.completed),
            width=(
                None
                if self.bar_width is None
                else max(1, self.bar_width)
            ),
...
class TotalFileSizeColumn(ProgressColumn):
    ...
    def render(self, task: "Task") -> Text:
        """Show data completed."""
        data_size = (
            filesize.decimal(int(task.total))
...
class DownloadColumn(ProgressColumn):
    ...
    def render(self, task: "Task") -> Text:
        ...
        download_text = Text(
            download_status, style="progress.download"
        )
    ...
    def speed(self) -> Optional[float]:
        ...
        with self._lock:
            ...
            if not progress:
                return None
            total_time = (
                progress[-1].timestamp - progress[0].timestamp
            )
            if total_time == 0:
                return None
...
class Progress(JupyterMixin):
    ...
    def track(
        self,
        sequence: Union[
            Iterable[ProgressType], Sequence[ProgressType]
        ],
        total: Optional[float] = None,
    ...
    def update(
        ...
        **fields: Any,
        ...
        with self._lock:
            ...
            current_time = self.get_time()
            old_sample_time = (
                current_time - self.speed_estimate_period
            )
            _progress = task._progress

            popleft = _progress.popleft
            while (
                _progress and _progress[0].timestamp < old_sample_time
            ):
                popleft()
