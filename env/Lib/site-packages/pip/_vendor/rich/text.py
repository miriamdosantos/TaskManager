...
class Text(JupyterMixin):
    ...
    def on(
        ...
        meta.update(
            {f"@{key}": value for key, value in handlers.items()}
        )
        ...
        for match in re.finditer(re_highlight, plain):
            ...
            for name in match.groupdict().keys():
                ...
                if start != -1 and end > start:
                    append_span(
                        _Span(start, end, f"{style_prefix}{name}")
                    )
        return count
    ...
    def render(
        ...
        def get_current_style() -> Style:
            ...
            styles = tuple(
                style_map[_style_id] for _style_id in sorted(stack)
            )
        ...
        for (offset, leaving, style_id), (next_offset, _, _) in zip(
            ...
            if next_offset > offset:
                yield _Segment(
                    text[offset:next_offset], get_current_style()
                )
        if end:
            yield _Segment(end)
    ...
    def join(self, lines: Iterable["Text"]) -> "Text":
        ...
        for text in iter_text():
            ...
            if text.style:
                append_span(
                    _Span(offset, offset + len(text), text.style)
                )
            extend_spans(
        ...
        if _overflow != "ignore":
            ...
            if length > max_width:
                if _overflow == "ellipsis":
                    self.plain = (
                        set_cell_size(self.plain, max_width - 1) + "…"
                    )
                else:
    ...
    def pad(self, count: int, character: str = " ") -> None:
        ...
        if count:
            ...
            self.plain = (
                f"{pad_characters}{self.plain}{pad_characters}"
            )
            _Span = Span
            self._spans[:] = [
    ...
    def append(
        ...
        if len(text):
            if isinstance(text, str):
                ...
                self._spans.extend(
                    _Span(
                        start + text_length, end + text_length, style
                    )
                    for start, end, style in text._spans
                )
    ...
    def append_text(self, text: "Text") -> "Text":
        ...
        if text.style is not None:
            self._spans.append(
                _Span(
                    text_length, text_length + len(text), text.style
                )
            )
        self._text.append(text.plain)
        self._spans.extend(
    ...
    def divide(self, offsets: Iterable[int]) -> Lines:
        ...
        for span_start, span_end, style in self._spans:
            ...
            for line_no in range(start_line_no, end_line_no + 1):
                ...
                new_end = min(
                    span_end - line_start, line_end - line_start
                )
    ...
    def wrap(
        ...
        *,
        ...
        no_wrap = (
            pick_bool(no_wrap, self.no_wrap, False)
            or overflow == "ignore"
        )
    ...
    def with_indent_guides(
        ...
        *,
        ...
        for line in text.split(allow_blank=True):
            ...
            new_indent = (
                f"{indent_line * full_indents}{' ' * remaining_space}"
            )
