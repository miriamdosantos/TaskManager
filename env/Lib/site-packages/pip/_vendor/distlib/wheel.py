...
class Wheel(object):
    ...
    def build(self, paths, tags=None, wheel_version=None):
        ...
        libkey = list(
            filter(lambda o: o in paths, ("purelib", "platlib"))
        )[0]
        if libkey == "platlib":
            is_pure = "false"
            ...
            if os.path.isdir(path):
                for root, dirs, files in os.walk(path):
                    for fn in files:
                        ...
                        archive_paths.append((ap, p))
                        if key == "scripts" and not p.endswith(
                            ".exe"
                        ):
                            with open(p, "rb") as f:
                                data = f.read()
        ...
        self.write_records(
            (distinfo, info_dir), libdir, archive_paths
        )
    ...
    def update(self, modifier, dest_dir=None, **kwargs):
        ...
        def update_version(version, path):
            ...
            try:
                ...
                if i < 0:
                    ...
                    parts = [
                        int(s) for s in version[i + 1 :].split(".")
                    ]
                    parts[-1] += 1
                    updated = "%s+%s" % (
                        version[:i],
        ...
        with tempdir() as workdir:
            ...
            if modified:
                ...
                current_version, path = get_version(
                    path_map, info_dir
                )
...
def is_compatible(wheel, tags=None):
    ...
    for ver, abi, arch in tags:
        if (
            ver in wheel.pyver
            and abi in wheel.abi
            and arch in wheel.arch
        ):
            result = True
            break
    return result
