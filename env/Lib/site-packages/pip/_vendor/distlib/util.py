...
def parse_marker(marker_string):
    ...
    def marker_var(remaining):
        ...
        if m:
            ...
            remaining = remaining[
                1:
            ].lstrip()  # skip past closing quote
    ...
    def copy_file(self, infile, outfile, check=True):
        ...
        if not self.dry_run:
            ...
            if check:
                ...
                elif os.path.exists(outfile) and not os.path.isfile(
                    outfile
                ):
    ...
    def copy_stream(self, instream, outfile, encoding=None):
        ...
        if not self.dry_run:
            if encoding is None:
                ...
                outstream = codecs.open(
                    outfile, "w", encoding=encoding
                )
            try:
...
def get_cache_base(suffix=None):
    ...
    if os.path.isdir(result):
        ...
        try:
            ...
            logger.warning(
                "Unable to create %s", result, exc_info=True
            )
            usable = False
    if not usable:
...
class Configurator(BaseConfigurator):
    ...
    def configure_custom(self, config):
        ...
        items = [
            (k, convert(config[k])) for k in config if valid_ident(k)
        ]
        kwargs = dict(items)
...
class SubprocessMixin(object):
    ...
    def run_command(self, cmd, **kwargs):
        ...
        t1 = threading.Thread(
            target=self.reader, args=(p.stdout, "stdout")
        )
        t1.start()
        t2 = threading.Thread(
            target=self.reader, args=(p.stderr, "stderr")
        )
        t2.start()
        p.wait()
        t1.join()
