...
class PackageIndex(object):
    ...
    def check_credentials(self):
        ...
        pm.add_password(
            self.realm, netloc, self.username, self.password
        )
    ...
    def run_command(self, cmd, input_data=None):
        ...
        t1 = Thread(
            target=self._reader, args=("stdout", p.stdout, stdout)
        )
        t1.start()
        t2 = Thread(
            target=self._reader, args=("stderr", p.stderr, stderr)
        )
        t2.start()
        if input_data is not None:
