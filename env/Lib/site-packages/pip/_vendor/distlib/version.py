...
class NormalizedVersion(Version):
    ...
    def parse(self, s):
        ...
        groups = m.groups()
        self._release_clause = tuple(
            int(v) for v in groups[1].split(".")
        )
        return result
...
class NormalizedMatcher(Matcher):
    ...
    def _adjust_local(self, version, constraint, prefix):
        if prefix:
            ...
            strip_local = (
                not constraint._parts[-1] and version._parts[-1]
            )
        if strip_local:
...
_REPLACEMENTS = (
    ...
    (
        re.compile(r"^v(ersion)?\s*(\d+)"),
        r"\2",
    ),  # remove leading v(ersion)
    (
        re.compile(r"^r(ev)?\s*(\d+)"),
        r"\2",
    ),  # remove leading v(ersion)
...
def _semantic_key(s):
    ...
    pre, build = make_tuple(groups[3], "|"), make_tuple(
        groups[5], "*"
    )
