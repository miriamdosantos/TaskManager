...
try:
    ...
    def _dnsname_match(dn, hostname, max_wildcards=1):
        ...
        if leftmost == "*":
            ...
            # fragment.
            pats.append("[^.]+")
        elif leftmost.startswith("xn--") or hostname.startswith(
            "xn--"
        ):
        ...
        pat = re.compile(
            r"\A" + r"\.".join(pats) + r"\Z", re.IGNORECASE
        )
            ...
            if bom_found:
                if codec.name != "utf-8":
                    ...
                    if filename is None:
                        ...
                        msg = (
                            "encoding problem for {!r}: utf-8".format(
                                filename
                            )
                        )
                    raise SyntaxError(msg)
    ...
    class ChainMap(MutableMapping):
        ...
        def __delitem__(self, key):
            try:
                ...
                raise KeyError(
                    "Key not found in the first mapping: {!r}".format(
                        key
                    )
                )
        ...
        def convert(self, value):
            ...
            if not isinstance(value, ConvertingDict) and isinstance(
                ...
                if m:
                    d = m.groupdict()
                    prefix = d["prefix"]
                    converter = self.value_converters.get(
                        prefix, None
                    )
                    if converter:
                        suffix = d["suffix"]
