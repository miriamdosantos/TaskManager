...
class DelegatingLexer(Lexer):
    ...
    def __init__(
        self, _root_lexer, _language_lexer, _needle=Other, **options
    ):
    ...
    def get_tokens_unprocessed(self, text):
        ...
        insertions = []
        lng_buffer = []
        for i, t, v in self.language_lexer.get_tokens_unprocessed(
            text
        ):
            if t is self.needle:
                if lng_buffer:
...
class words(Future):
    ...
    def get(self):
        return regex_opt(
            self.words, prefix=self.prefix, suffix=self.suffix
        )
...
class ProfilingRegexLexerMeta(RegexLexerMeta):
    ...
    def _process_regex(cls, regex, rflags, state):
        ...
        def match_func(text, pos, endpos=sys.maxsize):
            info = cls._prof_data[-1].setdefault(
                (state, rex), [0, 0.0]
            )
            t0 = time.time()
...
class ProfilingRegexLexer(
    ...
    def get_tokens_unprocessed(self, text, stack=("root",)):
        ...
        yield from RegexLexer.get_tokens_unprocessed(
            self, text, stack
        )
