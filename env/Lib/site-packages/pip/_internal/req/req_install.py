...
class InstallRequirement:
    ...
    def __init__(
        ...
        self.install_options = (
            install_options if install_options else []
        )
    ...
    def __str__(self) -> str:
        ...
        if self.satisfied_by is not None:
            s += " in {}".format(
                display_path(self.satisfied_by.location)
            )
        if self.comes_from:
    ...
    def hashes(self, trust_internet: bool = True) -> Hashes:
        ...
        if link and link.hash:
            good_hashes.setdefault(link.hash_name, []).append(
                link.hash
            )
    ...
    def _set_requirement(self) -> None:
        ...
        if isinstance(
            parse_version(self.metadata["Version"]), Version
        ):
            op = "=="
        else:
            op = "==="
    ...
    def update_editable(self) -> None:
        ...
        vcs_backend.obtain(
            self.source_dir, url=hidden_url, verbosity=0
        )
    ...
    def uninstall(
        ...
        if not dist:
            logger.warning(
                "Skipping %s as it is not installed.", self.name
            )
            return None
    ...
    def archive(self, build_dir: Optional[str]) -> None:
        ...
        if os.path.exists(archive_path):
            ...
            if response == "i":
                ...
                logger.warning(
                    "Deleting %s", display_path(archive_path)
                )
