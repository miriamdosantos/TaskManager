...
def _should_use_sysconfig() -> bool:
    ...
    return bool(
        getattr(
            sysconfig, "_PIP_USE_SYSCONFIG", _USE_SYSCONFIG_DEFAULT
        )
    )
...
def _looks_like_red_hat_patched_platlib_purelib(
    ...
    return (
        unpatched.replace("$platbase/", "$base/") == scheme["purelib"]
    )
...
def _looks_like_slackware_scheme() -> bool:
    ...
    try:
        ...
        return False
    return (
        "/lib64/" in paths["purelib"] and "/lib64/" not in user_site
    )
...
def _looks_like_msys2_mingw_scheme() -> bool:
    ...
    return all(
        "Lib" not in p
        and "lib" in p
        and not p.endswith("site-packages")
...
def _fix_abiflags(parts: Tuple[str]) -> Generator[str, None, None]:
    ...
    if (
        not ldversion
        or not abiflags
        or not ldversion.endswith(abiflags)
    ):
        yield from parts
        return
...
def get_purelib() -> str:
    ...
    if _looks_like_deb_system_dist_packages(old):
        return old
    if _warn_if_mismatch(
        pathlib.Path(old), pathlib.Path(new), key="purelib"
    ):
        _log_context()
    return old
...
def _looks_like_apple_library(path: str) -> bool:
    ...
    return (
        path
        == f"/Library/Python/{get_major_minor_version()}/site-packages"
    )
