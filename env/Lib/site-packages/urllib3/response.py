...
class BytesQueueBuffer:
    ...
    def get_all(self) -> bytes:
        ...
        if len(buffer) == 1:
            ...
            ret = io.BytesIO()
            ret.writelines(
                buffer.popleft() for _ in range(len(buffer))
            )
            result = ret.getvalue()
        self._size = 0
        return result
...
class BaseHTTPResponse(io.IOBase):
    ...
    def _flush_decoder(self) -> bytes:
        ...
        if self._decoder:
            return (
                self._decoder.decompress(b"") + self._decoder.flush()
            )
        return b""
...
class HTTPResponse(BaseHTTPResponse):
    ...
    def _init_length(self, request_method: str | None) -> int | None:
        ...
        """
        length: int | None
        content_length: str | None = self.headers.get(
            "content-length"
        )
        ...
        *,
        ...
        with self._error_catcher():
            ...
            if amt is not None and amt != 0 and not data:
                ...
                (amt != 0 and not data)
                or self.length_remaining == len(data)
            ):
    ...
    def stream(
        ...
        if self.chunked and self.supports_chunked_reads():
            ...
            while (
                ...
                data = self.read(
                    amt=amt, decode_content=decode_content
                )
