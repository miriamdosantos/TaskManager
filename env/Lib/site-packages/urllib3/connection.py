...
class HTTPConnection(_HTTPConnection):
    ...
    def _new_conn(self) -> socket.socket:
        ...
        if _HAS_SYS_AUDIT:
            sys.audit(
                "http.client.connect", self, self.host, self.port
            )
    ...
    def putheader(self, header: str, *values: str) -> None:  # type: ignore[override]
        """"""
        if not any(
            isinstance(v, str) and v == SKIP_HEADER for v in values
        ):
            ...
            skippable_headers = "', '".join(
                [
                    str.title(header)
                    for header in sorted(SKIPPABLE_HEADERS)
                ]
            )
            raise ValueError(
    ...
    def request_chunked(
        ...
        self.request(
            method, url, body=body, headers=headers, chunked=True
        )
...
class HTTPSConnection(HTTPConnection):
    ...
    def connect(self) -> None:
        ...
        if (
            self._tunnel_host is not None
            and self._tunnel_port is not None
        ):
        ...
        if target_supports_http2 is None:
            supports_http2 = (
                sock_and_verified.socket.selected_alpn_protocol()
                == "h2"
            )
            http2_probe.set_and_release(
                host=probe_http2_host,
        ...
        if (
            ...
            and self.proxy_is_verified is None
        ):
...
def _match_hostname(
    ...
    try:
        match_hostname(
            cert, asserted_hostname, hostname_checks_common_name
        )
...
def _url_from_connection(
    ...
    return Url(
        scheme=scheme, host=conn.host, port=conn.port, path=path
    ).url
