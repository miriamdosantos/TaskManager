...
def _is_has_never_check_common_name_reliable(
    ...
    is_openssl_issue_14579_fixed = (
        openssl_version_number >= 0x101010CF
    )
...
def create_urllib3_context(
    ...
    if ssl_version not in (None, PROTOCOL_TLS, PROTOCOL_TLS_CLIENT):
        ...
        # to avoid conflicts.
        if (
            ssl_minimum_version is not None
            or ssl_maximum_version is not None
        ):
            raise ValueError(
    ...
    try:
        ...
        AttributeError
    ):  # Defensive: for CPython < 3.8.9 and 3.9.3; for PyPy < 7.3.8
        pass
...
def ssl_wrap_socket(
    ...
    if (
        keyfile
        and key_password is None
        and _is_key_file_encrypted(keyfile)
    ):
        raise SSLError(
