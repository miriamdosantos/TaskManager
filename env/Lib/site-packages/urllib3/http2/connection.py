...
class HTTP2Connection(HTTPSConnection):
    def __init__(
        ...
        if (
            "proxy" in kwargs or "proxy_config" in kwargs
        ):  # Defensive:
            raise NotImplementedError(
                "Proxies aren't supported with HTTP/2"
            )
        ...
        if self._tunnel_host is not None:
            raise NotImplementedError(
                "Tunneling isn't supported with HTTP/2"
            )

    def _new_h2_conn(
        self,
    ) -> _LockedObject[h2.connection.H2Connection]:
        config = h2.config.H2Configuration(client_side=True)
        return _LockedObject(
            h2.connection.H2Connection(config=config)
        )
    ...
    def putrequest(  # type: ignore[override]
        ...
        **kwargs: typing.Any,
        ...
        if "skip_accept_encoding" in kwargs:
            raise NotImplementedError(
                "`skip_accept_encoding` isn't supported"
            )
    ...
    def putheader(
        ...
        header = (
            header.encode() if isinstance(header, str) else header
        )
        header = (
            header.lower()
        ...
        with self._h2_conn as conn:
            ...
            try:
                if isinstance(data, bytes):
                    conn.send_data(
                        self._h2_stream, data, end_stream=True
                    )
                    if data_to_send := conn.data_to_send():
                        self.sock.sendall(data_to_send)
                else:
