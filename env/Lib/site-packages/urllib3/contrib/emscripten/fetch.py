...
class _StreamingFetcher:
    ...
    def send(self, request: EmscriptenRequest) -> EmscriptenResponse:
        ...
        if js_int_buffer[0] == ERROR_TIMEOUT:
            ...
            json_str = js_decoder.decode(
                js_byte_buffer.slice(0, string_len)
            )
            # get it as an object
...
def is_in_browser_main_thread() -> bool:
    return (
        hasattr(js, "window")
        and hasattr(js, "self")
        and js.self == js.window
    )


def is_cross_origin_isolated() -> bool:
    return (
        hasattr(js, "crossOriginIsolated") and js.crossOriginIsolated
    )
...
def _show_timeout_warning() -> None:
    ...
    if not _SHOWN_TIMEOUT_WARNING:
        ...
        message = (
            "Warning: Timeout is not available on main browser thread"
        )
...
def send_request(request: EmscriptenRequest) -> EmscriptenResponse:
    try:
        ...
        headers = dict(
            Parser().parsestr(js_xhr.getAllResponseHeaders())
        )
