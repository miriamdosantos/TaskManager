...
def detailed_match_files(
    ...
    *all_matches* (:class:`bool` or :data:`None`) is whether to return all
    ...
    """
    all_files = (
        files if isinstance(files, CollectionType) else list(files)
    )
    return_files = {}
            ...
            if pattern.include:
                ...
                for result_file in result_files:
                    if result_file in return_files:
                        if all_matches:
                            return_files[result_file].patterns.append(
                                pattern
                            )
                        else:
                            return_files[result_file].patterns[
                                0
                            ] = pattern
                    else:
                        return_files[result_file] = MatchDetail(
                            [pattern]
                        )
    ...
    with os.scandir(dir_full) as scan_iter:
        ...
        for node_ent in scan_iter:
            ...
            if node_ent.is_dir(follow_symlinks=follow_links):
                ...
                # descendant files.
                yield TreeEntry(
                    node_ent.name, node_rel, node_lstat, node_stat
                )
...
def match_files(
    ...
    *files* (:class:`~collections.abc.Iterable` of :class:`str`) contains
    ...
    use_patterns = [
        __pat for __pat in patterns if __pat.include is not None
    ]
...
class TreeEntry(object):
    ...
    def stat(
        self, follow_links: Optional[bool] = None
    ) -> os.stat_result:
        """
