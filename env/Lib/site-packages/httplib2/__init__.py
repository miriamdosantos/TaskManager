    ...
    if maximum_version is not None:
        if hasattr(context, "maximum_version"):
            if isinstance(maximum_version, str):
                maximum_version = getattr(
                    ssl.TLSVersion, maximum_version
                )
        ...
        if hasattr(context, "minimum_version"):
            if isinstance(minimum_version, str):
                minimum_version = getattr(
                    ssl.TLSVersion, minimum_version
                )
...
def _entry_disposition(response_headers, request_headers):
    ...
    if (
        ...
        if "max-age" in cc_response:
            ...
            expires = email.utils.parsedate_tz(
                response_headers["expires"]
            )
            if None == expires:
                freshness_lifetime = 0
            else:
                freshness_lifetime = max(
                    0, calendar.timegm(expires) - date
                )
        else:
            freshness_lifetime = 0
        if "max-age" in cc:
        ...
        if encoding in ["gzip", "deflate"]:
            if encoding == "gzip":
                content = gzip.GzipFile(
                    fileobj=io.BytesIO(new_content)
                ).read()
            if encoding == "deflate":
                try:
    ...
    def _write_headers(self):
        ...
        for h, v in msg.items():
            ...
            if isinstance(v, header.Header):
                print(
                    v.encode(maxlinelen=self._maxheaderlen),
                    file=self._fp,
                )
            else:
    ...
    if cachekey:
        ...
        if "no-store" in cc or "no-store" in cc_response:
            ...
            try:
                ...
                setattr(
                    info, "_write_headers", _bind_write_headers(info)
                )
                header_str = info.as_string()
...
def _wsse_username_token(cnonce, iso_now, password):
    return (
        base64.b64encode(
            _sha(
                ("%s%s%s" % (cnonce, iso_now, password)).encode(
                    "utf-8"
                )
            ).digest()
        )
...
class DigestAuthentication(Authentication):
    ...
    def __init__(
        ...
        self.challenge = auth._parse_www_authenticate(
            response, "www-authenticate"
        )["digest"]
        qop = self.challenge.get("qop", "auth")
        self.challenge["qop"] = (
            ("auth" in [x.strip() for x in qop.split()])
            and "auth"
            or None
        )
    ...
    def request(
        ...
        if self.challenge.get("opaque"):
            headers["authorization"] += (
                ', opaque="%s"' % self.challenge["opaque"]
            )
        self.challenge["nc"] += 1

    def response(self, response, content):
        if "authentication-info" not in response:
            challenge = auth._parse_www_authenticate(
                response, "www-authenticate"
            ).get("digest", {})
            ...
            if "nextnonce" in updated_challenge:
                self.challenge["nonce"] = updated_challenge[
                    "nextnonce"
                ]
                self.challenge["nc"] = 1
        return False
...
class HmacDigestAuthentication(Authentication):
    ...
    def __init__(
        ...
        self.challenge["reason"] = self.challenge.get(
            "reason", "unauthorized"
        )
        if self.challenge["reason"] not in [
            "unauthorized",
            "integrity",
        ]:
        ...
        if self.challenge["algorithm"] not in [
            "HMAC-SHA-1",
            "HMAC-MD5",
        ]:
            raise UnimplementedHmacDigestAuthOptionError(
    ...
    def request(self, method, request_uri, headers, content):
        ...
        request_digest = (
            hmac.new(self.key, request_digest, self.hashmod)
            .hexdigest()
            .lower()
        )
        headers["authorization"] = (
...
class GoogleLoginAuthentication(Authentication):
    def __init__(
        ...
        lines = content.split("\n")
        d = dict(
            [tuple(line.split("=", 1)) for line in lines if line]
        )
        if resp.status == 403:
            self.Auth = ""
        else:
...
class ProxyInfo(object):
    ...
    def isgood(self):
        return (
            socks
            and (self.proxy_host != None)
            and (self.proxy_port != None)
        )
    ...
    def bypass_host(self, hostname):
        ...
        for skip_name in self.bypass_hosts:
            # *.suffix
            if skip_name.startswith(".") and hostname.endswith(
                skip_name
            ):
                return True
            # exact match
...
class Http(object):
    ...
    def _auth_from_challenge(
        ...
        """
        challenges = auth._parse_www_authenticate(
            response, "www-authenticate"
        )
    ...
    def _conn_request(self, conn, request_uri, method, body, headers):
        ...
        while i < RETRIES:
            ...
            try:
                ...
                if (
                    errno_ in (errno.ENETUNREACH, errno.EADDRNOTAVAIL)
                    and i < RETRIES
                ):
    ...
    def _request(
        ...
        if response.status == 401:
            for authorization in self._auth_from_challenge(
                ...
                authorization.request(
                    method, request_uri, headers, body
                )
                (response, content) = self._conn_request(
                    conn, request_uri, method, body, headers
                )
        ...
        if (
            ...
            if (
                ...
                and response.status in self.redirect_codes
            ):
                ...
                if redirections:
                    ...
                    if response.status == 308 or (
                        response.status == 301
                        and (method in self.safe_methods)
                    ):
    ...
    def request(
        ...
        try:
            ...
            if "user-agent" not in headers:
                headers["user-agent"] = (
                    "Python-httplib2/%s (gzip)" % __version__
                )
            ...
            (scheme, authority, request_uri, defrag_uri) = urlnorm(
                uri
            )
            ...
            if (
                ...
                if "-x-permanent-redirect-url" in info:
                    ...
                    entry_disposition = _entry_disposition(
                        info, headers
                    )
                ...
                if response.status == 304 and method == "GET":
                    ...
                    if hasattr(response, "_stale_digest"):
                        merged_response._stale_digest = (
                            response._stale_digest
                        )
                    _updateCache(
                        headers,
                        merged_response,
