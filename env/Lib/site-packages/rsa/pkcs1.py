...
def decrypt(crypto: bytes, priv_key: key.PrivateKey) -> bytes:
    ...
    cleartext_marker_bad = not compare_digest(
        cleartext[:2], b"\x00\x02"
    )
...
def sign_hash(
    hash_value: bytes, priv_key: key.PrivateKey, hash_method: str
) -> bytes:
...
def sign(
    message: bytes, priv_key: key.PrivateKey, hash_method: str
) -> bytes:
...
def find_signature_hash(
    signature: bytes, pub_key: key.PublicKey
) -> str:
...
def compute_hash(
    ...
    if isinstance(message, bytes):
        ...
        assert hasattr(message, "read") and hasattr(
            message.read, "__call__"
        )
        # read as 1K blocks
