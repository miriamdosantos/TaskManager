...
def keygen() -> None:
    ...
    if cli.pubout:
        print(
            "Writing public key to %s" % cli.pubout, file=sys.stderr
        )
...
class CryptoOperation(metaclass=abc.ABCMeta):
    ...
    key_class = (
        rsa.PublicKey
    )  # type: typing.Type[rsa.key.AbstractKey]
    ...
    def parse_cli(
        self,
    ) -> typing.Tuple[optparse.Values, typing.List[str]]:
        """Parse the CLI options
        ...
        parser.add_option(
            "-i", "--input", type="string", help=self.input_help
        )

        if self.has_output:
            parser.add_option(
                "-o", "--output", type="string", help=self.output_help
            )

        parser.add_option(
            "--keyform",
            help="Key format of the %s key - default PEM"
            % self.keyname,
            choices=("PEM", "DER"),
            default="PEM",
        )
    ...
    def read_key(
        self, filename: str, keyform: str
    ) -> rsa.key.AbstractKey:
        """Reads a public or private key."""

        print(
            "Reading %s key from %s" % (self.keyname, filename),
            file=sys.stderr,
        )
...
class SignOperation(CryptoOperation):
    ...
    def perform_operation(
        ...
        if hash_method not in HASH_METHODS:
            raise SystemExit(
                "Invalid hash method, choose one of %s"
                % ", ".join(HASH_METHODS)
            )
