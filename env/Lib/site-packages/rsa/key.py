...
class AbstractKey:
    ...
    @classmethod
    def load_pkcs1(
        cls: typing.Type[T], keyfile: bytes, format: str = "PEM"
    ) -> T:
    ...
    def _assert_format_exists(
        file_format: str,
        methods: typing.Mapping[str, typing.Callable],
    ) -> typing.Callable:
    ...
    def _update_blinding_factor(self) -> typing.Tuple[int, int]:
        ...
        with self.mutex:
            if self.blindfac < 0:
                ...
                self.blindfac_inverse = pow(
                    self.blindfac_inverse, 2, self.n
                )
...
class PublicKey(AbstractKey):
    ...
    def _load_pkcs1_der(cls, keyfile: bytes) -> "PublicKey":
        ...
        return cls(
            n=int(priv["modulus"]), e=int(priv["publicExponent"])
        )
    ...
    def load_pkcs1_openssl_der(cls, keyfile: bytes) -> "PublicKey":
        ...
        (keyinfo, _) = decoder.decode(
            keyfile, asn1Spec=OpenSSLPubKey()
        )

        if keyinfo["header"]["oid"] != univ.ObjectIdentifier(
...
class PrivateKey(AbstractKey):
    ...
    def __init__(
        self, n: int, e: int, d: int, p: int, q: int
    ) -> None:
    ...
    def __setstate__(
        self,
        state: typing.Tuple[int, int, int, int, int, int, int, int],
    ) -> None:
        """Sets the key from tuple."""
        (
            self.n,
...
def newkeys(
    ...
    (p, q, e, d) = gen_keys(
        nbits, getprime_func, accurate=accurate, exponent=exponent
    )
