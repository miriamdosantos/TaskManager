...
def validate(config: dict, filepath: _Path) -> bool:
    ...
    trove_classifier = validator.FORMAT_FUNCTIONS.get(
        "trove-classifier"
    )
...
def apply_configuration(
    ...
    config = read_configuration(
        filepath, True, ignore_option_errors, dist
    )
...
def read_configuration(
    ...
    if not os.path.isfile(filepath):
        raise FileError(
            f"Configuration file {filepath!r} does not exist."
        )
    ...
    if not asdict or not (project_table or setuptools_table):
        return (
            {}
        )  # User is not using pyproject to configure setuptools
    ...
    try:
        ...
        # TODO: Remove the following once the feature stabilizes:
        if _skip_bad_config(
            project_table, orig_setuptools_table, dist
        ):
            return {}
        ...
        if ignore_option_errors:
            _logger.debug(
                f"ignored error: {ex.__class__.__name__} - {ex}"
            )
        else:
...
def _skip_bad_config(
    ...
    if given_config <= popular_subset:
        ...
        warnings.warn(
            _InvalidFile.message(), _InvalidFile, stacklevel=2
        )
        return True
...
class _ConfigExpander:
    def __init__(
        ...
        self.setuptools_cfg = config.get("tool", {}).get(
            "setuptools", {}
        )
    ...
    def _process_field(
        self, container: dict, field: str, fn: Callable
    ):
    ...
    def expand(self):
        ...
        with _EnsurePackagesDiscovered(
            dist, self.setuptools_cfg
    ...
    def _expand_data_files(self):
        ...
        self._process_field(
            self.setuptools_cfg, "data-files", data_files
        )
    ...
    def _expand_all_dynamic(
        ...
        obtained_dynamic.update(
            ...
            optional_dependencies=self._obtain_optional_dependencies(
                dist
            ),
        )
        ...
        updates = {
            k: v for k, v in obtained_dynamic.items() if v is not None
        }
        self.project_cfg.update(updates)

    def _ensure_previously_set(
        self, dist: "Distribution", field: str
    ):
    ...
    def _obtain_version(
        ...
        if (
            ...
            and "version" in self.dynamic_cfg
        ):
...
class _EnsurePackagesDiscovered(_expand.EnsurePackagesDiscovered):
    def __init__(
        self, distribution: "Distribution", setuptools_cfg: dict
    ):
    ...
    def __enter__(self):
        ...
        package_dir: Dict[str, str] = cfg.setdefault(
            "package-dir", {}
        )
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        ...
        self._setuptools_cfg.setdefault(
            "packages", self._dist.packages
        )
        self._setuptools_cfg.setdefault(
            "py-modules", self._dist.py_modules
        )
