    ...
    for value in patterns:
        ...
        if any(char in value for char in glob_characters):
            ...
            expanded_values.extend(
                sorted(
                    os.path.relpath(path, root_dir).replace(
                        os.sep, "/"
                    )
            ...
            path = os.path.relpath(value, root_dir).replace(
                os.sep, "/"
            )
...
def _find_spec(
    module_name: str, module_path: Optional[_Path]
) -> ModuleSpec:
...
def _find_module(
    ...
    module_path = next(
        (x for x in candidates if os.path.isfile(x)), None
    )
...
def cmdclass(
    ...
    """
    return {
        k: resolve_class(v, package_dir, root_dir)
    ...
    **kwargs,
    ...
    if namespaces:
        from setuptools.discovery import (
            PEP420PackageFinder as PackageFinder,
        )
    else:
    ...
    packages: List[str] = []
    fill_package_dir = (
        {} if fill_package_dir is None else fill_package_dir
    )
...
def version(
    value: Union[Callable, Iterable[Union[str, int]], str]
) -> str:
...
def entry_points(
    text: str, text_source="entry-points"
) -> Dict[str, dict]:
...
class LazyMappingProxy(Mapping[_K, _V]):
    ...
    def __init__(
        self, obtain_mapping_value: Callable[[], Mapping[_K, _V]]
    ):
