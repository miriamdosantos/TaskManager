...
from setuptools._deprecation_warning import (
...
def _apply_project_table(
    dist: "Distribution", config: dict, root_dir: _Path
):
...
def _handle_missing_dynamic(
    dist: "Distribution", project_table: dict
):
...
def _guess_content_type(file: str) -> Optional[str]:
    ...
    msg = (
        f"only the following file extensions are recognized: {valid}."
    )
    raise ValueError(f"Undefined content type for {file}, {msg}")


def _long_description(
    dist: "Distribution", val: _DictOrStr, root_dir: _Path
):
...
def _license(dist: "Distribution", val: dict, root_dir: _Path):
    ...
    if "file" in val:
        _set_config(
            dist,
            "license",
            expand.read_files([val["file"]], root_dir),
        )
    else:
...
def _optional_dependencies(
    dist: "Distribution", val: dict, _root_dir
):
...
def _unify_entry_points(project_table: dict):
    ...
    entry_points = project.pop(
        "entry-points", project.pop("entry_points", {})
    )
    renaming = {
...
def _copy_command_options(
    ...
    for cmd, config in (
        pyproject.get("tool", {}).get("distutils", {}).items()
    ):
        ...
        for key, value in config.items():
            ...
            if key not in valid:
                ...
                # just log a warn...
                _logger.warning(
                    f"Command option {cmd}.{key} is not defined"
                )


def _valid_command_options(
    cmdclass: Mapping = EMPTY,
) -> Dict[str, Set[str]]:
    ...
    unloaded_entry_points = metadata.entry_points(
        group="distutils.commands"
    )
    loaded_entry_points = (
        _load_ep(ep) for ep in unloaded_entry_points
    )
...
def _normalise_cmd_options(
    ...
    return {
        _normalise_cmd_option_key(fancy_option[0])
        for fancy_option in desc
    }
...
_PREVIOUSLY_DEFINED = {
    ...
    "authors": _some_attrgetter(
        "metadata.author", "metadata.author_email"
    ),
    ...
    "dependencies": _some_attrgetter(
        "_orig_install_requires", "install_requires"
    ),
    "optional-dependencies": _some_attrgetter(
        "_orig_extras_require", "extras_require"
    ),
}
