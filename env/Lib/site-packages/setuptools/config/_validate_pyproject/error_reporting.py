...
class ValidationError(JsonSchemaValueException):
    ...
    def _from_jsonschema(cls, ex: JsonSchemaValueException):
        ...
        if debug_code != "false":  # pragma: no cover
            obj.__cause__, obj.__traceback__ = (
                ex.__cause__,
                ex.__traceback__,
            )
...
class _ErrorFormatting:
    def __init__(self, ex: JsonSchemaValueException):
        ...
        self._original_message = self.ex.message.replace(
            ex.name, self.name
        )
        self._summary = ""
        self._details = ""
...
class _SummaryWriter:
        ...
        *,
        ...
        with io.StringIO() as buffer:
            for i, (key, value) in enumerate(filtered.items()):
                ...
                buffer.write(
                    f"{line_prefix}{self._label(child_path)}:"
                )
                # ^  just the first item should receive the complete prefix
                if isinstance(value, dict):
                    filtered = self._filter_unecessary(
                        value, child_path
                    )
                    simple = self._handle_simple_dict(
                        filtered, child_path
                    )
                    buffer.write(
                        f" {simple}"
                    ...
                    buffer.write(
                        f" {self._value(value, child_path)}\n"
                    )
    ...
    def _is_unecessary(self, path: Sequence[str]) -> bool:
        ...
        key = path[-1]
        return (
            any(key.startswith(k) for k in "$_")
    ...
    def _handle_simple_dict(
        ...
        if inline or simple:
            return (
                f"{{{', '.join(self._inline_attrs(value, path))}}}\n"
            )
        return None
    ...
    def _label(self, path: Sequence[str]) -> str:
        *parents, key = path
        if not self._is_property(path):
            ...
            return self._terms.get(key) or " ".join(
                self._jargon(norm_key)
            )
    ...
    def _child_prefix(
        self, parent_prefix: str, child_prefix: str
    ) -> str:
