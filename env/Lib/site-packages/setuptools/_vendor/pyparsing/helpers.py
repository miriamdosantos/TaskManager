    ...
    *,
    ...
    dlName = "{expr} [{delim} {expr}]...{end}".format(
        ...
        delim=str(delim),
            ...
            " [{}]".format(str(delim)) if allow_trailing_delim else ""
        ),
    )
...
def counted_array(
    ...
    *,
    ...
    intExpr.add_parse_action(
        count_field_parse_action, call_during_try=True
    )
    return (intExpr + array_expr).set_name(
        "(len) " + str(expr) + "..."
    )
...
def match_previous_expr(expr: ParserElement) -> ParserElement:
    ...
    def copy_token_to_repeater(s, l, t):
        ...
        rep.set_parse_action(
            must_match_these_tokens, callDuringTry=True
        )
...
def one_of(
    ...
    *,
    ...
    if caseless:
        ...
        parseElementClass = (
            CaselessKeyword if asKeyword else CaselessLiteral
        )
    else:
    ...
    if useRegex:
        ...
        try:
            ...
            ret = Regex(patt, flags=re_flags).set_name(
                " | ".join(symbols)
            )
...
def dict_of(
    key: ParserElement, value: ParserElement
) -> ParserElement:
...
def original_text_for(
    ...
    *,
    ...
    matchExpr = (
        locMarker("_original_start")
        + expr
        + endlocMarker("_original_end")
    )
    if asString:
        extractText = lambda s, l, t: s[
            t._original_start : t._original_end
        ]
    else:
...
def nested_expr(
    ...
    *,
    ...
    if content is None:
        if isinstance(opener, str_type) and isinstance(
            closer, str_type
        ):
...
def _makeTags(
    ...
    if xml:
        tagAttrValue = dbl_quoted_string.copy().set_parse_action(
            remove_quotes
        )
        openTag = (
            suppress_LT
    ...
    openTag.add_parse_action(
        lambda t: t.__setitem__(
            "start"
            + "".join(resname.replace(":", " ").title().split()),
            t.copy(),
        )
    )
    closeTag = closeTag(
...
def infix_notation(
    ...
    if not (
        isinstance(rpar, Suppress) and isinstance(rpar, Suppress)
    ):
    ...
    for i, operDef in enumerate(op_list):
        ...
        if arity == 3:
            if (
                not isinstance(opExpr, (tuple, list))
                or len(opExpr) != 2
            ):
                raise ValueError(
        ...
        if rightLeftAssoc is OpAssoc.LEFT:
            ...
            if arity == 1:
                ...
                if opExpr is not None:
                    ...
                    ) + Group(lastExpr + (opExpr + thisExpr)[1, ...])
                else:
...
def indentedBlock(
    ...
    UNDENT = (
        Empty().set_parse_action(checkUnindent).set_name("UNINDENT")
    )
    if indent:
        smExpr = Group(
            Opt(NL)
    ...
    smExpr.add_parse_action(
        lambda: (
            backup_stacks.pop(-1) and None if backup_stacks else None
        )
    )
...
rest_of_line = (
    Regex(r".*").leave_whitespace().set_name("rest of line")
)
