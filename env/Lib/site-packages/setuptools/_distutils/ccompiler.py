...
class CCompiler:
    ...
    def _need_link(self, objects, output_file):
        ...
        if self.force:
            ...
            if self.dry_run:
                newer = newer_group(
                    objects, output_file, missing="newer"
                )
            else:
...
"""
    ...
    def shared_object_filename(
        self, basename, strip_dir=0, output_dir=""
    ):
        assert output_dir is not None
        if strip_dir:
            basename = os.path.basename(basename)
        return os.path.join(
            output_dir, basename + self.shared_lib_extension
        )
    ...
    def executable_filename(
        ...
        if strip_dir:
            basename = os.path.basename(basename)
        return os.path.join(
            output_dir, basename + (self.exe_extension or "")
        )
...
def new_compiler(
    ...
    try:
        ...
        (module_name, class_name, long_description) = compiler_class[
            compiler
        ]
    except KeyError:
        msg = (
