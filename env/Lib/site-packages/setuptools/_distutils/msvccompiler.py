...
class MacroExpander:
    ...
    def load_macros(self, version):
        ...
        self.set_macro(
            "VCInstallDir", vsbase + r"\Setup\VC", "productdir"
        )
        self.set_macro(
            "VSInstallDir", vsbase + r"\Setup\VS", "productdir"
        )
...
class MSVCCompiler(CCompiler):
    ...
    def __init__(self, verbose=0, dry_run=0, force=0):
        ...
        if self.__arch == "Intel":
            ...
            self.__product = (
                "Visual Studio version %s" % self.__version
            )
        else:
            # Win64. Assume this was built with the platform SDK
            self.__product = "Microsoft SDK compiler %s" % (
                self.__version + 6
            )
    ...
    def get_msvc_paths(self, path, platform="x86"):
        ...
        if self.__version == 6:
            for base in HKEYS:
                if (
                    read_values(base, r"%s\6.0" % self.__root)
                    is not None
                ):
                    self.warn(
