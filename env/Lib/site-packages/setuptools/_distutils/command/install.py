...
def _load_schemes():
    ...
    return {
        ...
        for scheme in set(
            itertools.chain(INSTALL_SCHEMES, sysconfig_schemes)
        )
    }
...
def _select_scheme(ob, name):
    scheme = _inject_headers(
        name, _load_scheme(_resolve_scheme(name))
    )
...
class install(Command):
    ...
    def finalize_options(self):
        ...
        (prefix, exec_prefix) = get_config_vars(
            "prefix", "exec_prefix"
        )
        try:
        ...
        self.install_lib = os.path.join(
            self.install_lib, self.extra_dirs
        )
    ...
    def finalize_unix(self):
        ...
        if (
            self.install_base is not None
            or self.install_platbase is not None
        ):
            incomplete_scheme = (
                (
    ...
    def change_roots(self, *names):
        ...
        for name in names:
            ...
            setattr(
                self,
                attr,
                change_root(self.root, getattr(self, attr)),
            )
    ...
    def run(self):
        ...
        if not self.skip_build:
            ...
            build_plat = self.distribution.get_command_obj(
                "build"
            ).plat_name
        ...
        install_lib = os.path.normcase(
            os.path.normpath(self.install_lib)
        )
        if (
            self.warn_dir
