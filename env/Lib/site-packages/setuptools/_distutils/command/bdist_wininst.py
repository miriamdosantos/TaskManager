...
class bdist_wininst(Command):
    ...
    def get_inidata(self):
        ...
        lines.append(
            "target_compile=%d" % (not self.no_target_compile)
        )
        lines.append(
            "target_optimize=%d" % (not self.no_target_optimize)
        )
        if self.target_version:
            lines.append("target_version=%s" % self.target_version)
        if self.user_access_control:
            lines.append(
                "user_access_control=%s" % self.user_access_control
            )
    ...
    def get_exe_bytes(self):
        ...
        filename = os.path.join(
            directory, "wininst-%s%s.exe" % (bv, sfix)
        )
        f = open(filename, "rb")
        try:
            return f.read()
