...
class build_ext(Command):
        ...
        if os.name == "nt":
            ...
            if self.debug:
                self.build_temp = os.path.join(
                    self.build_temp, "Debug"
                )
            else:
                self.build_temp = os.path.join(
                    self.build_temp, "Release"
                )

            # Append the source distribution include and library directories,
            # this allows distutils on windows to work in the source tree
            self.include_dirs.append(
                os.path.dirname(get_config_h_filename())
            )
    ...
    def run(self):
        ...
        if self.distribution.has_c_libraries():
            ...
            self.libraries.extend(
                build_clib.get_library_names() or []
            )
    ...
    def check_extensions_list(self, extensions):
        ...
        for i, ext in enumerate(extensions):
            ...
            if macros:
                ...
                for macro in macros:
                    if not (
                        isinstance(macro, tuple)
                        and len(macro) in (1, 2)
                    ):
                        raise DistutilsSetupError(
    ...
    def _filter_build_errors(self, ext):
        try:
            ...
            if not ext.optional:
                raise
            self.warn(
                'building extension "%s" failed: %s' % (ext.name, e)
            )
    ...
    def build_extension(self, ext):
        ...
        if not (
            self.force or newer_group(depends, ext_path, "newer")
        ):
            log.debug(
                "skipping '%s' extension (up-to-date)", ext.name
            )
            return
        else:
    ...
    def get_export_symbols(self, ext):
        ...
        try:
            ...
            suffix = "U_" + name.encode("punycode").replace(
                b"-", b"_"
            ).decode("ascii")
        else:
            suffix = "_" + name
