...
def _msvc14_find_vc2017():
    ...
    installed.
    """
    root = environ.get("ProgramFiles(x86)") or environ.get(
        "ProgramFiles"
    )
    if not root:
        return None, None
...
def _augment_exception(exc, version, arch=""):
    ...
    if (
        "vcvarsall" in message.lower()
        or "visual c" in message.lower()
    ):
...
class SystemInfo:
    ...
    def VSInstallDir(self):
        ...
        return (
            self.ri.lookup(self.ri.vs, "%0.1f" % self.vs_ver)
            or default
        )
    ...
    def _guess_vc_legacy(self):
        ...
        # Try to get path from registry, if fail use default path
        return (
            self.ri.lookup(self.ri.vc, "%0.1f" % self.vs_ver)
            or default_vc
        )
    ...
    def FrameworkDir32(self):
        ...
        # Try to get path from registry, if fail use default path
        return (
            self.ri.lookup(self.ri.vc, "frameworkdir32") or guess_fw
        )
    ...
    def FrameworkDir64(self):
        ...
        # Try to get path from registry, if fail use default path
        return (
            self.ri.lookup(self.ri.vc, "frameworkdir64") or guess_fw
        )
    ...
    def _find_dot_net_versions(self, bits):
        ...
        ver = (
            reg_ver or self._use_last_dir_name(dot_net_dir, "v") or ""
        )
...
class EnvironmentInfo:
    ...
    def FxTools(self):
        ...
        if self.vs_ver <= 10.0:
            include32 = True
            include64 = (
                not pi.target_is_x86() and not pi.current_is_x86()
            )
        else:
            include32 = pi.target_is_x86() or pi.current_is_x86()
            include64 = (
                pi.current_cpu == "amd64" or pi.target_cpu == "amd64"
            )
