...
def read_pkg_file(self, file):
    ...
    self.long_description = _read_field_unescaped_from_msg(
        msg, "description"
    )
    if (
...
def check_package_data(dist, attr, value):
    ...
    for k, v in value.items():
        if not isinstance(k, str):
            raise DistutilsSetupError(
                "keys of {!r} dict must be strings (got {!r})".format(
                    attr, k
                )
            )
        assert_string_list(
            dist, "values of {!r} dict".format(attr), v
        )
...
class Distribution(_Distribution):
    ...
    def _set_metadata_defaults(self, attrs):
        ...
        for (
            option,
            default,
        ) in self._DISTUTILS_UNSUPPORTED_METADATA.items():
    ...
    def _finalize_requires(self):
        ...
        if getattr(self, "extras_require", None):
            ...
            self._orig_extras_require = (
                self._orig_extras_require or self.extras_require
            )
        ...
        if (
            getattr(self, "install_requires", None)
            and not self._orig_install_requires
        ):
    ...
    def _convert_extras_requirements(self):
        ...
        self._tmp_extras_require = getattr(
            self, "_tmp_extras_require", tmp
        )
    ...
    def _move_install_requirements_markers(self):
        ...
        self.extras_require = dict(
            ...
            (
                ...
                list(
                    dict.fromkeys(
                        str(r) for r in map(self._clean_req, v)
                    )
                ),
            )
    ...
    def _finalize_license_files(self):
        ...
        license_files: Optional[List[str]] = (
    ...
    def _set_command_options(
        ...
        if DEBUG:
            self.announce(
                "  setting options for '%s' command:" % command_name
            )
    ...
    def _get_project_config_files(self, filenames):
        ...
        if filenames is not None:
            parts = partition(
                lambda f: Path(f).suffix == ".toml", filenames
            )
            filenames = list(
                parts[0]
            )  # 1st element => predicate is False
            tomlfiles = list(
                parts[1]
            )  # 2nd element => predicate is True
    ...
    def parse_config_files(
        ...
        """
        inifiles, tomlfiles = self._get_project_config_files(
            filenames
        )
    ...
    def get_command_class(self, command):
        ...
        eps = metadata.entry_points(
            group="distutils.commands", name=command
        )
        for ep in eps:
