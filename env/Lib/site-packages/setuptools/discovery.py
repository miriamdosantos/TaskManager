...
class _Finder:
    ...
    def _build_filter(*patterns: str) -> _Filter:
        ...
        return lambda name: any(
            fnmatchcase(name, pat) for pat in patterns
        )
...
class FlatLayoutPackageFinder(PEP420PackageFinder):
    ...
    DEFAULT_EXCLUDE = tuple(
        chain_iter((p, f"{p}.*") for p in _EXCLUDE)
    )
    ...
    def _looks_like_package(_path: _Path, package_name: str) -> bool:
        ...
        # Consider PEP 561
        root_pkg_is_valid = names[0].isidentifier() or names[
            0
        ].endswith("-stubs")
...
class ConfigDiscovery:
    ...
    def __call__(
        self, force=False, name=True, ignore_ext_modules=False
    ):
    ...
    def _explicitly_specified(self, ignore_ext_modules: bool) -> bool:
        ...
        ignore_ext_modules = (
            ignore_ext_modules or self._skip_ext_modules
        )
        ext_modules = not (
            self.dist.ext_modules is None or ignore_ext_modules
        )
        return (
    ...
    def _analyse_package_layout(
        self, ignore_ext_modules: bool
    ) -> bool:
    ...
    def _analyse_explicit_layout(self) -> bool:
        ...
        package_dir = (
            self._package_dir.copy()
        )  # don't modify directly
        package_dir.pop(
            "", None
        )  # This falls under the "src-layout" umbrella
        ...
        log.debug(
            f"`explicit-layout` detected -- analysing {package_dir}"
        )
        pkgs = chain_iter(
            _find_packages_within(
                pkg, os.path.join(root_dir, parent_dir)
            )
    ...
    def _analyse_src_layout(self) -> bool:
        ...
        src_dir = os.path.join(
            self._root_dir, package_dir.get("", "src")
        )
        ...
        self.dist.package_dir = (
            package_dir  # persist eventual modifications
        )
    ...
    def _analyse_flat_layout(self) -> bool:
        ...
        """
        log.debug(
            f"`flat-layout` detected -- analysing {self._root_dir}"
        )
        return (
            self._analyse_flat_packages()
            or self._analyse_flat_modules()
        )

    def _analyse_flat_packages(self) -> bool:
        self.dist.packages = FlatLayoutPackageFinder.find(
            self._root_dir
        )
        top_level = remove_nested_packages(
            remove_stubs(self.dist.packages)
        )
        log.debug(f"discovered packages -- {self.dist.packages}")
    ...
    def _analyse_flat_modules(self) -> bool:
        self.dist.py_modules = FlatLayoutModuleFinder.find(
            self._root_dir
        )
        log.debug(f"discovered py_modules -- {self.dist.py_modules}")
        self._ensure_no_accidental_inclusion(
            self.dist.py_modules, "modules"
        )
        return bool(self.dist.py_modules)

    def _ensure_no_accidental_inclusion(
        self, detected: List[str], kind: str
    ):
        if len(detected) > 1:
    ...
    def _find_name_single_package_or_module(self) -> Optional[str]:
        ...
        for field in ("packages", "py_modules"):
            ...
            if items and len(items) == 1:
                log.debug(
                    f"Single module/package detected, name: {items[0]}"
                )
                return items[0]
    ...
    def _find_name_from_packages(self) -> Optional[str]:
        ...
        parent_pkg = find_parent_package(
            packages, package_dir, self._root_dir
        )
        if parent_pkg:
            log.debug(
                f"Common parent package detected, name: {parent_pkg}"
            )
            return parent_pkg
...
def remove_stubs(packages: List[str]) -> List[str]:
    ...
    return [
        pkg
        for pkg in packages
        if not pkg.split(".")[0].endswith("-stubs")
    ]
...
def find_parent_package(
    ...
    for i, name in enumerate(packages):
        if not all(
            n.startswith(f"{name}.") for n in packages[i + 1 :]
        ):
...
def find_package_path(
    name: str, package_dir: Dict[str, str], root_dir: _Path
) -> str:
