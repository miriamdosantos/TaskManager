...
class Credentials(_BaseCredentials):
    ...
    def _non_blocking_refresh(self, request):
        ...
        if self.token_state == TokenState.STALE:
            use_blocking_refresh_fallback = (
                not self._refresh_worker.start_refresh(self, request)
            )

        if (
            self.token_state == TokenState.INVALID
            or use_blocking_refresh_fallback
        ):
            self.refresh(request)
    ...
    def before_request(self, request, method, url, headers):
        ...
        metrics.add_metric_header(
            headers, self._metric_header_for_usage()
        )
        self.apply(headers)
...
class CredentialsWithTokenUri(Credentials):
    ...
    def with_token_uri(self, token_uri):
        ...
        """
        raise NotImplementedError(
            "This credential does not use token uri."
        )
...
class Scoped(ReadOnlyScoped):
    ...
    def with_scopes(self, scopes, default_scopes=None):
        ...
        """
        raise NotImplementedError(
            "This class does not require scoping."
        )
...
def with_scopes_if_required(credentials, scopes, default_scopes=None):
    ...
    """
    if (
        isinstance(credentials, Scoped)
        and credentials.requires_scopes
    ):
