...
_GCE_METADATA_HOST = os.getenv(
    environment_vars.GCE_METADATA_HOST, None
)
...
try:
    _METADATA_DEFAULT_TIMEOUT = int(
        os.getenv("GCE_METADATA_TIMEOUT", 3)
    )
...
def ping(request, timeout=_METADATA_DEFAULT_TIMEOUT, retry_count=3):
    ...
    for attempt in backoff:
        try:
            ...
            metadata_flavor = response.headers.get(
...
def get(
    ...
    for attempt in backoff:
        try:
            response = request(
                url=url, method="GET", headers=headers_to_use
            )
            if (
                response.status
        ...
        raise exceptions.TransportError(
            ...
            "metadata service. Compute Engine Metadata server unavailable".format(
                url
            )
        )
    ...
    if (
        ...
        and return_none_for_not_found_error
    ):
        return None

    if response.status == http_client.OK:
        if (
            _helpers.parse_content_type(
...
def get_service_account_token(
    request, service_account="default", scopes=None
):
