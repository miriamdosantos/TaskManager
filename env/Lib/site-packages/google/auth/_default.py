...
def load_credentials_from_dict(
    ...
    if not isinstance(info, dict):
        raise exceptions.DefaultCredentialsError(
            "info object was of type {} but dict type was expected.".format(
                type(info)
            )
        )
...
def _load_credentials_from_info(
    ...
    if credential_type == _AUTHORIZED_USER_TYPE:
        ...
        credentials, project_id = (
            _get_gdch_service_account_credentials(filename, info)
        )
    else:
        raise exceptions.DefaultCredentialsError(
    ...
    if isinstance(credentials, CredentialsWithQuotaProject):
        credentials = _apply_quota_project_id(
...
def _get_explicit_environ_credentials(quota_project_id=None):
    ...
    if (
        explicit_file is not None
        and explicit_file == cloud_sdk_adc_path
    ):
...
def _get_gae_credentials():
    ...
    if (
        os.environ.get(environment_vars.LEGACY_APPENGINE_RUNTIME)
        != "python27"
    ):
        return None, None
...
def _get_gce_credentials(request=None, quota_project_id=None):
    ...
    try:
        ...
        _LOGGER.warning(
            "Import of Compute Engine auth library failed."
        )
        return None, None
...
def _get_external_account_credentials(
    ...
    if info.get("subject_token_type") == _AWS_SUBJECT_TOKEN_TYPE:
        ...
        and info.get("credential_source").get("executable")
        is not None
    ):
...
def _get_service_account_credentials(
    filename, info, scopes=None, default_scopes=None
):
...
def _get_impersonated_service_account_credentials(
    filename, info, scopes
):
    ...
    try:
        ...
        if (
            ...
            raise exceptions.InvalidValue(
                "Cannot extract target principal from {}".format(
                    impersonation_url
                )
            )
        target_principal = impersonation_url[
...
def _get_gdch_service_account_credentials(filename, info):
    ...
    try:
        ...
        msg = "Failed to load GDCH service account credentials from {}".format(
            filename
        )
        new_exc = exceptions.DefaultCredentialsError(msg, caught_exc)
        raise new_exc from caught_exc
...
def _apply_quota_project_id(credentials, quota_project_id):
    if quota_project_id:
        ...
        credentials = (
            credentials.with_quota_project_from_environment()
        )

    from google.oauth2 import (
        credentials as authorized_user_credentials,
    )

    if isinstance(
        credentials, authorized_user_credentials.Credentials
    ) and (not credentials.quota_project_id):
...
def default(
    ...
    checkers = (
        ...
        lambda: _get_gcloud_sdk_credentials(
            quota_project_id=quota_project_id
        ),
