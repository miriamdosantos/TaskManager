    ...
    def get_subject_token(self, context, request):
        if not os.path.exists(self._path):
            raise exceptions.RefreshError(
                "File '{}' was not found.".format(self._path)
            )
        ...
        return _parse_token_data(
            token_content,
            self._format_type,
            self._subject_token_field_name,
        )
...
class _UrlSupplier(SubjectTokenSupplier):
    ...
    def __init__(
        self, url, format_type, subject_token_field_name, headers
    ):
        self._url = url
        self._format_type = format_type
        self._subject_token_field_name = subject_token_field_name
    ...
    def get_subject_token(self, context, request):
        response = request(
            url=self._url, method="GET", headers=self._headers
        )
...
class Credentials(external_account.Credentials):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if (
            ...
            and subject_token_supplier is None
        ):
        ...
        if (
            ...
            and subject_token_supplier is not None
        ):
            raise exceptions.InvalidValue(
    ...
    def _validate_certificate_config(self):
        self._certificate_config_location = (
            self._credential_source_certificate.get(
    ...
    def _validate_file_or_url_config(self, credential_source):
        self._credential_source_headers = credential_source.get(
            "headers"
        )
        ...
        if self._credential_source_format_type not in [
            "text",
            "json",
        ]:
            raise exceptions.MalformedError(
    ...
    def _validate_single_source(self):
        ...
        valid_credential_sources = list(
            filter(
                lambda source: source is not None, credential_sources
            )
        )
