...
class Credentials(external_account.Credentials):
    ...
    def retrieve_subject_token(self, request):
        ...
        if self._credential_source_executable_output_file is not None:
            try:
                ...
                try:
                    ...
                    subject_token = self._parse_subject_token(
                        response
                    )
                    if (
        ...
        exe_timeout = (
            ...
            / 1000
            if self.interactive
            else self._credential_source_executable_timeout_millis
            / 1000
        )
        exe_stdin = sys.stdin if self.interactive else None
        exe_stdout = (
            sys.stdout if self.interactive else subprocess.PIPE
        )
        exe_stderr = (
            sys.stdout if self.interactive else subprocess.STDOUT
        )
    ...
    def _parse_subject_token(self, response):
        ...
        if (
            "expiration_time" in response
            and response["expiration_time"] < time.time()
        ):
            raise exceptions.RefreshError(
    ...
    def _validate_running_mode(self):
        ...
        if (
            self.interactive
            and not self._credential_source_executable_output_file
        ):
            raise exceptions.MalformedError(
