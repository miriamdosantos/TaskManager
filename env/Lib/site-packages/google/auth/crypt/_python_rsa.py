...
def _bit_list_to_bytes(bit_list):
    ...
    for start in range(0, num_bits, 8):
        ...
        char_val = sum(
            val * digit for val, digit in zip(_POW2, curr_bits)
        )
    ...
    def from_string(cls, public_key):
        ...
        if is_x509_cert:
            ...
            asn1_cert, remaining = decoder.decode(
                der, asn1Spec=Certificate()
            )
            if remaining != b"":
                raise exceptions.InvalidValue(
                    "Unused bytes", remaining
                )

            cert_info = asn1_cert["tbsCertificate"][
                "subjectPublicKeyInfo"
            ]
            key_bytes = _bit_list_to_bytes(
...
class RSASigner(base.Signer, base.FromServiceAccountMixin):
    ...
    def from_string(cls, key, key_id=None):
        ...
        # Key is in pkcs8.
            ...
            private_key_info = key_info.getComponentByName(
                "privateKey"
            )
            private_key = rsa.key.PrivateKey.load_pkcs1(
