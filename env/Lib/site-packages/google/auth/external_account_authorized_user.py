...
class Credentials(
    ...
    def info(self):
        ...
        config_info.update(
            type=_EXTERNAL_ACCOUNT_AUTHORIZED_USER_JSON_TYPE
        )
        if config_info["expiry"]:
            config_info["expiry"] = (
                config_info["expiry"].isoformat() + "Z"
            )
    ...
    def to_json(self, strip=None):
        ...
        return json.dumps(
            {k: v for (k, v) in self.info.items() if k not in strip}
        )
    ...
    def _make_sts_request(self, request):
        return self._sts_client.refresh_token(
