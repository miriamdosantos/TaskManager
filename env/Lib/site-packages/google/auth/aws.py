...
class RequestSigner(object):
    ...
    def get_request_options(
        ...
        header_map = _generate_authentication_header_map(
            ...
            canonical_querystring=_get_canonical_querystring(
                uri.query
            ),
            method=method,
        ...
        if aws_security_credentials.session_token is not None:
            headers[_AWS_SECURITY_TOKEN_HEADER] = (
...
def _get_canonical_querystring(query):
    ...
    for key in sorted_keys:
        for item in querystring_encoded_map[key]:
            querystring_encoded_pairs.append(
                "{}={}".format(key, item)
            )
...
def _generate_authentication_header_map(
    ...
    payload_hash = hashlib.sha256(
        (request_payload or "").encode("utf-8")
    ).hexdigest()
    ...
    signing_key = _get_signing_key(
        aws_security_credentials.secret_access_key,
        date_stamp,
        region,
        service_name,
    )
...
class _DefaultAwsSecurityCredentialsSupplier(
    ...
    def get_aws_region(self, context, request):
        ...
        if imdsv2_session_token is not None:
            headers = {
                "X-aws-ec2-metadata-token": imdsv2_session_token
            }

        response = request(
            url=self._region_url, method="GET", headers=headers
        )
        ...
        if response.status != http_client.OK:
            raise exceptions.RefreshError(
                "Unable to retrieve AWS region: {}".format(
                    response_body
                )
            )
    ...
    def _get_imdsv2_session_token(self, request):
        if (
            ...
            imdsv2_session_token_response = request(
                url=self._imdsv2_session_token_url,
                method="PUT",
                headers=headers,
            )
    ...
    def _get_metadata_security_credentials(
        ...
        response = request(
            url="{}/{}".format(
                self._security_credentials_url, role_name
            ),
            method="GET",
            headers=headers,
        )
        ...
        if response.status != http_client.OK:
            raise exceptions.RefreshError(
                "Unable to retrieve AWS security credentials: {}".format(
                    response_body
                )
            )
    ...
    def _get_metadata_role_name(self, request, imdsv2_session_token):
        ...
        if response.status != http_client.OK:
            raise exceptions.RefreshError(
                "Unable to retrieve AWS role name {}".format(
                    response_body
                )
            )
...
class Credentials(external_account.Credentials):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if (
            ...
            and aws_security_credentials_supplier is None
        ):
        ...
        if aws_security_credentials_supplier:
            ...
            environment_id = (
                credential_source.get("environment_id") or ""
            )
            self._aws_security_credentials_supplier = (
    ...
    def retrieve_subject_token(self, request):
        ...
        request_options = self._request_signer.get_request_options(
            aws_security_credentials,
            self._cred_verification_url.replace(
                "{region}", self._region
            ),
            "POST",
        )
        ...
        request_headers["x-goog-cloud-target-resource"] = (
        ...
        return urllib.parse.quote(
            json.dumps(
                aws_signed_req, separators=(",", ":"), sort_keys=True
            )
        )

    def _create_default_metrics_options(self):
        metrics_options = super(
            Credentials, self
