...
def encode(signer, payload, header=None, key_id=None):
    ...
    if "alg" not in header:
        if es256 is not None and isinstance(
            signer, es256.ES256Signer
        ):
...
def _unverified_decode(token):
    ...
    if not isinstance(header, Mapping):
        raise exceptions.MalformedError(
            "Header segment should be a JSON object: {0}".format(
                encoded_header
            )
        )
    ...
    if not isinstance(payload, Mapping):
        raise exceptions.MalformedError(
            "Payload segment should be a JSON object: {0}".format(
                encoded_payload
            )
        )
...
def _verify_iat_and_exp(payload, clock_skew_in_seconds=0):
    ...
    if latest < now:
        raise exceptions.InvalidValue(
            "Token expired, {} < {}".format(latest, now)
        )
...
def decode(
    ...
    Raises:
        ...
        google.auth.exceptions.MalformedError: if schema validation failed.
    """
    header, payload, signed_section, signature = _unverified_decode(
        token
    )
    ...
    if isinstance(certs, Mapping):
        if key_id:
            if key_id not in certs:
                raise exceptions.MalformedError(
                    "Certificate for key id {} not found.".format(
                        key_id
                    )
                )
...
class Credentials(
    ...
    def from_signing_credentials(
        cls, credentials, audience, **kwargs
    ):
    ...
    def with_claims(
        ...
        return self.__class__(
            ...
            audience=(
                audience if audience is not None else self._audience
            ),
            additional_claims=new_additional_claims,
            quota_project_id=self._quota_project_id,
        )

    @_helpers.copy_docstring(
        google.auth.credentials.CredentialsWithQuotaProject
    )
...
class OnDemandCredentials(
    ...
    def with_claims(
        self, issuer=None, subject=None, additional_claims=None
    ):
        """Returns a copy of these credentials with modified claims.
