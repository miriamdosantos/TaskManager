...
class ClientAuthentication(object):
    ...
    def __init__(
        self, client_auth_type, client_id, client_secret=None
    ):
...
class OAuthClientAuthHandler(metaclass=abc.ABCMeta):
    ...
    def _inject_authenticated_headers(
        self, headers, bearer_token=None
    ):
        ...
        if (
            ...
            if request_body is None:
                ...
                request_body["client_id"] = (
                    self._client_authentication.client_id
                )
...
def handle_error_response(response_body):
    ...
    try:
        ...
        error_components.append(
            "Error code {}".format(error_data["error"])
        )
        if "error_description" in error_data:
            error_components.append(
                ": {}".format(error_data["error_description"])
            )
        if "error_uri" in error_data:
            error_components.append(
                " - {}".format(error_data["error_uri"])
            )
