...
class Credentials(
    ...
    def __init__(
        ...
        self._universe_domain = (
            universe_domain or credentials.DEFAULT_UNIVERSE_DOMAIN
        )
    ...
    def __setstate__(self, d):
        ...
        self._universe_domain = (
            d.get("_universe_domain")
            or credentials.DEFAULT_UNIVERSE_DOMAIN
        )
    ...
    def refresh(self, request):
        ...
        if self._refresh_token is None and self.refresh_handler:
            token, expiry = self.refresh_handler(
            ...
            if (
                _helpers.utcnow()
                >= expiry - _helpers.REFRESH_THRESHOLD
            ):
                raise exceptions.RefreshError(
...
class UserAccessTokenCredentials(
    ...
    def with_account(self, account):
        ...
        return self.__class__(
            account=account, quota_project_id=self._quota_project_id
        )

    @_helpers.copy_docstring(credentials.CredentialsWithQuotaProject)
    def with_quota_project(self, quota_project_id):
        return self.__class__(
            account=self._account, quota_project_id=quota_project_id
        )
