...
def _handle_error_response(response_data, retryable_error):
    ...
    if isinstance(response_data, str):
        raise exceptions.RefreshError(
            response_data, retryable=retryable_error
        )
    try:
...
def _can_retry(status_code, response_data):
    ...
    try:
        ...
        if any(
            e in retryable_error_descriptions
            for e in (error_code, error_desc)
        ):
            return True
...
def _token_endpoint_request_no_throw(
    ...
    **kwargs
    ...
    if access_token:
        headers_to_use["Authorization"] = "Bearer {}".format(
            access_token
        )
...
def refresh_grant(
    ...
    return _handle_refresh_grant_response(
