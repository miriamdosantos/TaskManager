...
class LineGenerator(Visitor[Line]):
    ...
    def visit_simple_stmt(self, node: Node) -> Iterator[Line]:
        ...
        if node.parent and node.parent.type in STATEMENT:
            if is_parent_function_or_class(node) and is_stub_body(
                node
            ):
                yield from self.visit_default(node)
            else:
    ...
    def visit_async_stmt(self, node: Node) -> Iterator[Line]:
        ...
        for child in children:
            ...
            if (
                ...
                or child.type == STANDALONE_COMMENT
            ):
    ...
    def visit_STRING(self, leaf: Leaf) -> Iterator[Line]:
        ...
        if is_docstring(leaf, self.mode) and not re.search(
            ...
            if quote_len == 3:
                ...
                if (
                    len(lines) > 1
                    and last_line_length + quote_len
...
def _hugging_power_ops_line_to_string(
    ...
    try:
        return line_to_string(
            next(hug_power_op(line, features, mode))
        )
...
def transform_line(
    ...
    line_str_hugging_power_ops = (
        _hugging_power_ops_line_to_string(line, features, mode)
        or line_str
    )
    ...
    if (
        ...
        if Preview.string_processing in mode:
            ...
            transformers = []
    elif line.is_def and not should_split_funcdef_with_rhs(
        line, mode
    ):
...
def should_split_funcdef_with_rhs(line: Line, mode: Mode) -> bool:
    ...
    leaves_to_track = get_leaves_inside_matching_brackets(
        return_type_leaves
    )
...
def left_hand_split(
    ...
    for leaf in line.leaves:
        if (
            ...
            current_leaves = (
                tail_leaves if body_leaves else head_leaves
            )
...
def _first_right_hand_split(
    ...
    for leaf in reversed(line.leaves):
        if current_leaves is body_leaves:
            if leaf is opening_bracket:
                current_leaves = (
                    head_leaves if body_leaves else tail_leaves
                )
        current_leaves.append(leaf)
    ...
    bracket_split_succeeded_or_raise(head, body, tail)
    return RHSResult(
        head, body, tail, opening_bracket, closing_bracket
    )
    ...
    if (
        ...
        try:
            ...
            rhs_head_short_enough = is_line_short_enough(
                rhs.head,
                mode=replace(mode, line_length=mode.line_length - 1),
            )
...
def delimiter_split(
    ...
    try:
        delimiter_priority = bt.max_delimiter_priority(
    ...
    for leaf_idx, leaf in enumerate(line.leaves):
        ...
        if trailing_comma_safe and leaf.bracket_depth == lowest_depth:
            trailing_comma_safe = _can_add_trailing_comma(
                leaf, features
            )
    ...
    if (
        ...
        if (
            ...
            if maybe_make_parens_invisible_in_atom(
                ...
                wrap_in_parentheses(
                    node, node.children[1], visible=False
                )
            ...
            closing_bracket = cast(
                Leaf, node.children[1].children[-1]
            )
...
def _maybe_wrap_cms_in_parens(
    ...
    if colon_index is not None:
        ...
        new_child = Node(
            syms.atom,
            [lpar, Node(syms.testlist_gexp, context_managers), rpar],
        )
...
def should_split_line(line: Line, opening_bracket: Leaf) -> bool:
    ...
    if not (
        opening_bracket.parent and opening_bracket.value in "[{("
    ):
        return False
...
def generate_trailers_to_omit(
    ...
    for index, leaf, leaf_length in line.enumerate_with_length(
        ...
        has_inline_comment = leaf_length > len(leaf.value) + len(
            leaf.prefix
        )
