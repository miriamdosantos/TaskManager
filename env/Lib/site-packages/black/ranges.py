...
class _TopLevelStatementsVisitor(Visitor[None]):
    ...
    def visit_simple_stmt(self, node: Node) -> Iterator[None]:
        ...
        if not _get_line_range(ancestor).intersection(
            self._lines_set
        ):
...
def _convert_unchanged_line_by_line(
    ...
    for leaf in node.leaves():
        ...
        if leaf.parent and leaf.parent.type == syms.match_stmt:
            ...
            if not _get_line_range(nodes_to_ignore).intersection(
                lines_set
            ):
            ...
            while (
                parent_sibling
                and not parent_sibling.type == syms.suite
            ):
...
def _get_line_range(node_or_nodes: Union[LN, list[LN]]) -> set[int]:
    ...
    if isinstance(node_or_nodes, list):
        ...
        if isinstance(node, Leaf):
            ...
            if first and last:
                return set(
                    range(first.lineno, _leaf_line_end(last) + 1)
                )
            else:
                return set()
