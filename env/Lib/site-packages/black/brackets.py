...
class BracketTracker:
    ...
    def any_open_for_or_lambda(self) -> bool:
        ...
        for details."""
        return bool(
            self._for_loop_depths or self._lambda_argument_depths
        )
    ...
    def max_delimiter_priority(
        ...
        """
        return max(
            v for k, v in self.delimiters.items() if k not in exclude
        )
    ...
    def delimiter_count_with_priority(
        ...
        return sum(
            1 for p in self.delimiters.values() if p == priority
        )
...
def is_split_before_delimiter(
    ...
    if (
        ...
        not in {syms.import_from, syms.dotted_name}
