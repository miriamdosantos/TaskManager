...
def is_docstring(node: NL, mode: Mode) -> bool:
    ...
    if prev_siblings_are(
        node.parent,
        [None, token.NEWLINE, token.INDENT, syms.simple_stmt],
    ):
        return True
...
def is_one_tuple(node: LN) -> bool:
    ...
    if node.type == syms.atom:
        ...
        return (
            len(gexp.children) == 2
            and gexp.children[1].type == token.COMMA
        )
...
def is_tuple_containing_walrus(node: LN) -> bool:
    ...
    return any(
        child.type == syms.namedexpr_test for child in gexp.children
    )
...
def fstring_to_string(node: Node) -> Leaf:
    ...
    string_leaf = Leaf(
        token.STRING, string_without_prefix, prefix=node.prefix
    )
...
def is_stub_suite(node: Node) -> bool:
    ...
    if node.parent is not None and not is_parent_function_or_class(
        node
    ):
        return False
...
def is_stub_body(node: LN) -> bool:
    ...
    return (
        ...
        and all(
            leaf == Leaf(token.DOT, ".") for leaf in child.children
        )
    )
