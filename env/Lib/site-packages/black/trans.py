...
Transformer = Callable[
    [Line, Collection[Feature], Mode], Iterator[Line]
]
Index = int
NodeType = int
ParserState = int
...
def hug_power_op(
    ...
    def is_simple_operand(index: int, kind: Literal[1, -1]) -> bool:
        ...
        if start.type in {token.PLUS, token.MINUS, token.TILDE}:
            if line.leaves[index + 1].type in {
                token.NAME,
                token.NUMBER,
            }:
...
class StringMerger(StringTransformer, CustomSplitMapMixin):
    ...
    def do_transform(
        ...
        if isinstance(rblc_result, Err) and isinstance(
            msg_result, Err
        ):
    ...
    def _merge_string_group(
        ...
        for string_idx in string_indices:
            ...
            if isinstance(vresult, Err):
                continue
            merged_string_idx_dict[string_idx] = (
    ...
    def _merge_one_string_group(
        ...
        for has_prefix in prefix_tracker:
            ...
            breakpoint_idx = (
                mark_idx + (len(prefix) if has_prefix else 0) + 1
            )
            custom_splits.append(
                CustomSplit(has_prefix, breakpoint_idx)
            )

        string_leaf = Leaf(
            token.STRING, S_leaf.value.replace(BREAK_MARK, "")
        )
...
class StringParenStripper(StringTransformer):
    ...
    def do_match(self, line: Line) -> TMatchResult:
        ...
        while True:
            ...
            if (
                ...
                if is_valid_index(next_idx + 1) and LL[
                    next_idx + 1
                ].type in {
                    token.DOUBLESTAR,
                    token.LSQB,
    ...
    def _transform_to_new_line(
        ...
        for idx in sorted(string_and_rpar_indices):
            leaf = LL[idx]
            lpar_or_rpar_idx = (
                idx - 1 if leaf.type == token.STRING else idx
            )
            append_leaves(
                new_line,
                line,
...
class StringSplitter(BaseStringSplitter, CustomSplitMapMixin):
    ...
    def do_transform(
        ...
        def maybe_append_string_operators(new_line: Line) -> None:
            ...
            """
            maybe_prefix_leaves = (
                string_op_leaves if first_string_line else []
            )
        ...
        use_custom_breakpoints = bool(
            custom_splits
            and all(
                csplit.break_idx <= max_break_width
        ...
        def more_splits_should_be_made() -> bool:
            ...
            if use_custom_breakpoints:
                ...
                return (
                    str_width(rest_value) > max_last_string_column()
                )
        ...
        if is_valid_index(string_idx + 1):
            ...
            # else on a line below that...
                ...
                append_leaves(
                    non_string_line, line, LL[string_idx + 1 :]
                )
    ...
    def _get_break_idx(
        ...
        def passes_all_checks(i: Index) -> bool:
            ...
            is_split_safe = (
                is_valid_index(i - 1)
                and string[i - 1] in SPLIT_SAFE_CHARS
            )
        ...
        if not passes_all_checks(break_idx):
            ...
            while is_valid_index(
                break_idx + 1
            ) and not passes_all_checks(break_idx):
                break_idx += 1
...
class StringParenWrapper(BaseStringSplitter, CustomSplitMapMixin):
    ...
    def do_splitter_match(self, line: Line) -> TMatchResult:
        ...
        if string_idx is not None:
            ...
            if not any(
                ...
                max_string_width = self.line_length - (
                    (line.depth + 1) * 4
                )
    ...
    def _return_match(LL: list[Leaf]) -> Optional[int]:
        ...
        if parent_type(LL[0]) in [
            ...
            idx = (
                2 if is_valid_index(1) and is_empty_par(LL[1]) else 1
            )
    ...
    def _assert_match(LL: list[Leaf]) -> Optional[int]:
        ...
        if (
            parent_type(LL[0]) == syms.assert_stmt
            and LL[0].value == "assert"
        ):
            is_valid_index = is_valid_index_factory(LL)
    ...
    def _assign_match(LL: list[Leaf]) -> Optional[int]:
        ...
        if (
            parent_type(LL[0])
            in [syms.expr_stmt, syms.argument, syms.power]
            and LL[0].type == token.NAME
        ):
            is_valid_index = is_valid_index_factory(LL)
    ...
    def _dict_or_lambda_match(LL: list[Leaf]) -> Optional[int]:
        ...
        if (
            syms.dictsetmaker in parent_types
            or syms.lambdef in parent_types
        ):
            is_valid_index = is_valid_index_factory(LL)
    ...
    def do_transform(
        ...
        if is_valid_index(string_idx + 1):
            ...
            if old_parens_exist:
                assert (
                    right_leaves
                    and right_leaves[-1].type == token.RPAR
                ), (
            ...
            if (current_state, next_token) in self._goto:
                ...
                if (current_state, self.DEFAULT_TOKEN) in self._goto:
                    self._state = self._goto[
                        current_state, self.DEFAULT_TOKEN
                    ]
