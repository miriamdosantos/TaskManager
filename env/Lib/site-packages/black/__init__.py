...
def read_pyproject_toml(
    ...
    if extend_exclude is not None and not isinstance(
        extend_exclude, str
    ):
        raise click.BadOptionUsage(
            "extend-exclude",
    ...
    for s in src:
        ...
        if is_stdin or path.is_file():
            if resolves_outside_root_or_cannot_stat(
                ...
                if verbose:
                    out(
                        f'Skipping invalid source: "{path}"', fg="red"
                    )
                continue
    ...
    *,
    ...
    try:
        dst = format_file_contents(
            src, fast=fast, mode=mode, lines=lines
        )
        return True
...
def format_cell(src: str, *, fast: bool, mode: Mode) -> str:
    ...
    try:
        masked_src, replacements = mask_cell(
...
def get_features_used(  # noqa: C901
    ...
    for n in node.pre_order():
        if n.type == token.FSTRING_START:
            ...
            n.type
            in (syms.typevartuple, syms.paramspec, syms.typevar)
