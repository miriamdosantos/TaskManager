...
class Line:
    ...
    def is_comment(self) -> bool:
        ...
        return (
            len(self.leaves) == 1
            and self.leaves[0].type == STANDALONE_COMMENT
        )
    ...
    def is_with_or_async_with_stmt(self) -> bool:
        ...
        return bool(self) and is_with_or_async_with_stmt(
            self.leaves[0]
        )
    ...
    def is_stub_def(self) -> bool:
        ...
        return self.is_def and self.leaves[-4:] == [
            Leaf(token.COLON, ":")
        ] + [Leaf(token.DOT, ".") for _ in range(3)]
    ...
    def is_chained_assignment(self) -> bool:
        ...
        return [leaf.type for leaf in self.leaves].count(
            token.EQUAL
        ) > 1
    ...
    def is_fmt_pass_converted(
        self,
        *,
        first_leaf_matches: Optional[Callable[[Leaf], bool]] = None,
    ) -> bool:
    ...
    def contains_unsplittable_type_ignore(self) -> bool:
        ...
        first_line = next(
            (leaf.lineno for leaf in self.leaves if leaf.lineno != 0),
            0,
        )
        last_line = next(
            (
    ...
    def remove_trailing_comma(self) -> None:
        ...
        trailing_comma_comments = self.comments.pop(
            id(trailing_comma), []
        )
    ...
    def __str__(self) -> str:
        ...
        for leaf in leaves:
            res += str(leaf)
        for comment in itertools.chain.from_iterable(
...
class LinesBlock:
    ...
    def all_lines(self) -> list[str]:
        ...
        prefix = make_simple_prefix(
            self.before, self.form_feed, empty_line
        )
        return (
            [prefix] + self.content_lines + [empty_line * self.after]
        )
...
class EmptyLineTracker:
    ...
    def _maybe_empty_lines(
        ...
        while (
            self.previous_defs
            and self.previous_defs[-1].depth >= depth
        ):
    ...
    if leaves[0].type == token.STRING and leaves[1].type == token.DOT:
        ...
        for leaf in leaves[-2::-1]:
            if leaf.type in OPENING_BRACKETS:
                ...
                if not (
                    next.type == token.DOT
                    or next.type in OPENING_BRACKETS
                ):
                    return False
...
def can_omit_invisible_parens(
    ...
    for leaf in reversed(line.leaves):
        if (
            closing_bracket
            and leaf is closing_bracket.opening_bracket
        ):
    ...
    if (
        ...
        if _can_omit_opening_paren(
            line, first=first, line_length=line_length
        ):
            return True
    ...
    if (
        ...
        if _can_omit_closing_paren(
            line, last=last, line_length=line_length
        ):
            return True
