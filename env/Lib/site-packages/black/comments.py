...
class ProtoComment:
    ...
    * `newlines` and `consumed` fields are kept separate from the `value`. This
    ...
    leading_whitespace: (
        str  # leading whitespace before the comment, if any
    )
...
def convert_one_fmt_off_pair(
    ...
    for leaf in node.leaves():
        ...
        for comment in list_comments(leaf.prefix, is_endmarker=False):
            ...
            is_fmt_skip = _contains_fmt_skip_comment(
                comment.value, mode
            )
            ...
            ignored_nodes = list(
                generate_ignored_nodes(leaf, comment, mode)
            )
...
def is_fmt_on(container: LN) -> bool:
    ...
    for comment in list_comments(
        container.prefix, is_endmarker=False
    ):
...
def _contains_fmt_skip_comment(comment_line: str, mode: Mode) -> bool:
    ...
    return any(
        comment in FMT_SKIP for comment in semantic_comment_blocks
    )
