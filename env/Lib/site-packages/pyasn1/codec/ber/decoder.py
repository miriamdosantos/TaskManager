...
class AbstractPayloadDecoder(object):
    ...
    def indefLenValueDecoder(
        ...
        **options,
        ...
        """
        raise error.PyAsn1Error(
            "Indefinite length mode decoder not implemented for %s"
            % (tagSet,)
        )  # TODO: Seems more like an NotImplementedError?
...
class IntegerPayloadDecoder(AbstractSimplePayloadDecoder):
    ...
    def valueDecoder(
        ...
        **options,
        ...
        yield self._createComponent(
            asn1Spec, tagSet, value, **options
        )
...
class BitStringPayloadDecoder(AbstractSimplePayloadDecoder):
    ...
    def valueDecoder(
        ...
        **options,
        ...
        if (
            tagSet[0].tagFormat == tag.tagFormatSimple
        ...
        yield self._createComponent(
            asn1Spec, tagSet, bitString, **options
        )
...
class OctetStringPayloadDecoder(AbstractSimplePayloadDecoder):
    ...
    def valueDecoder(
        ...
        **options,
        ...
        yield self._createComponent(
            asn1Spec, tagSet, header, **options
        )
    ...
    def indefLenValueDecoder(
        ...
        **options,
        ...
        if (
            substrateFun
            and substrateFun is not self.substrateCollector
        ):
            asn1Object = self._createComponent(
                asn1Spec, tagSet, noValue, **options
            )
        ...
        **options,
        ...
        while index < substrateLen:
            ...
            if subId < 128:
                ...
                # page 7
                raise error.PyAsn1Error(
                    "Invalid octet 0x80 in OID encoding"
                )
        ...
        if 0 <= oid[0] <= 39:
            ...
            raise error.PyAsn1Error(
                "Malformed first OID octet: %s" % chunk[0]
            )
...
class RelativeOIDPayloadDecoder(AbstractSimplePayloadDecoder):
    ...
    def valueDecoder(
        ...
        **options,
        ...
        yield self._createComponent(
            asn1Spec, tagSet, reloid, **options
        )
...
class ConstructedPayloadDecoderBase(
    ...
    def _decodeComponentsSchemaless(
        ...
        **options,
        ...
        while (
            length == -1
            or substrate.tell() < original_position + length
        ):
        ...
        **options,
        ...
        if substrateFun:
            if asn1Spec is not None:
                ...
                asn1Object = (
                    self.protoRecordComponent,
                    self.protoSequenceComponent,
                )
        ...
        if asn1Spec is None:
            ...
            if substrate.tell() < original_position + length:
                if LOG:
                    for trailing in readFromStream(
                        substrate, context=options
                    ):
                        if isinstance(
                            trailing, SubstrateUnderrunError
                        ):
                            yield trailing
        ...
        if asn1Spec.typeId in (univ.Sequence.typeId, univ.Set.typeId):
            ...
            isDeterministic = (
                not isSetType and not namedTypes.hasOptionalOrDefault
            )
            ...
            while substrate.tell() - original_position < length:
                if not namedTypes:
                    ...
                    try:
                        if isDeterministic:
                            ...
                            namedTypes[idx].isOptional
                            or namedTypes[idx].isDefaulted
                        ):
                            componentType = (
                                namedTypes.getTagMapNearPosition(idx)
                            )
                ...
                for component in decodeFun(
                    substrate, componentType, **options
                ):
                    if isinstance(component, SubstrateUnderrunError):
                        yield component
            ...
            if namedTypes:
                if not namedTypes.requiredComponents.issubset(
                    seenIndices
                ):
                    raise error.PyAsn1Error(
                ...
                if namedTypes.hasOpenTypes:
                    ...
                    if openTypes or options.get(
                        "decodeOpenTypes", False
                    ):

                        for idx, namedType in enumerate(
                            ...
                            if (
                                ...
                                and not asn1Object.getComponentByPosition(
                                    idx
                                ).isValue
                            ):
                                continue
                            ...
                            try:
                                ...
                                if LOG:
                                    ...
                                    for (
                                        k,
                                        v,
                                    ) in namedType.openType.items():
                                        LOG("%s -> %r" % (k, v))

                                try:
                                    openType = namedType.openType[
                                        governingValue
                                    ]
                            ...
                            containerValue = (
                                asn1Object.getComponentByPosition(idx)
                            )
            ...
            if LOG:
                LOG(
                    "decoding type %r chosen by given `asn1Spec`"
                    % componentType
                )
...
class SequenceOrSequenceOfPayloadDecoder(
    ...
    def valueDecoder(
        ...
        **options,
        ...
        if asn1Object.tagSet == tagSet:
            if LOG:
                LOG(
                    "decoding %s as explicitly tagged CHOICE"
                    % (tagSet,)
                )
...
class SingleItemDecoder(object):
    ...
    def __init__(self, tagMap=_MISSING, typeMap=_MISSING, **ignored):
        self._tagMap = (
            tagMap if tagMap is not _MISSING else self.TAG_MAP
        )
        self._typeMap = (
            typeMap if typeMap is not _MISSING else self.TYPE_MAP
        )
    ...
    def __call__(
        ...
        **options,
        ...
        if allowEoo and self.supportIndefLength:
            ...
            for eoo_candidate in readFromStream(
                substrate, 2, options
            ):
        ...
        while state is not stStop:
            ...
            if state is stDecodeTag:
                ...
                for firstByte in readFromStream(
                    substrate, 1, options
                ):
                ...
                try:
                    ...
                    if tagId == 0x1F:
                        ...
                        while True:
                            for integerByte in readFromStream(
                                substrate, 1, options
                            ):
                                if isinstance(
                                    integerByte,
                ...
                if LOG:
                    LOG(
                        "tag decoded into %s, decoding length"
                        % tagSet
                    )

            if state is stDecodeLength:
                # Decode length
                for firstOctet in readFromStream(
                    substrate, 1, options
                ):
                ...
                if firstOctet < 128:
                    ...
                    for encodedLength in readFromStream(
                        substrate, size, options
                    ):
                        if isinstance(
                            encodedLength, SubstrateUnderrunError
                        ):
                            yield encodedLength
                ...
                if asn1Spec.__class__ is tagmap.TagMap:
                    ...
                    if LOG:
                        ...
                        for (
                            firstOctet,
                            v,
                        ) in asn1Spec.presentTypes.items():
                            LOG(
                                "  %s -> %s"
            ...
            if state is stDecodeValue:
                if (
                    not options.get("recursiveFlag", True)
                    and not substrateFun
                ):  # deprecate this

                    def substrateFun(
                        asn1Object, _substrate, _length, _options
                    ):
...
def __getattr__(attr: str):
    if newAttr := {"tagMap": "TAG_MAP", "typeMap": "TYPE_MAP"}.get(
        attr
    ):
        warnings.warn(
