...
class AbstractItemEncoder(object):
    ...
    def encode(self, value, asn1Spec=None, encodeFun=None, **options):
        ...
        for idx, singleTag in enumerate(tagSet.superTags):
            ...
            if not idx:
                try:
                    substrate, isConstructed, isOctets = (
                        self.encodeValue(
                            value, asn1Spec, encodeFun, **options
                        )
                    )
                ...
                if (
                    not substrate
                    and isConstructed
                    and options.get("ifNotEmpty", False)
                ):
                    return substrate
            ...
            header += self.encodeLength(
                len(substrate), defModeOverride
            )
...
class BitStringEncoder(AbstractItemEncoder):
    def encodeValue(self, value, asn1Spec, encodeFun, **options):
        ...
        if not maxChunkSize or len(alignedValue) <= maxChunkSize * 8:
            ...
            return (
                bytes((len(substrate) * 8 - valueLength,))
                + substrate,
                False,
                True,
            )
        ...
        while stop < valueLength:
            ...
            substrate += encodeFun(
                alignedValue[start:stop], asn1Spec, **options
            )
    ...
    def encodeValue(self, value, asn1Spec, encodeFun, **options):
        ...
        for subOid in value.asTuple():
            if 0 <= subOid <= 127:
                ...
                raise error.PyAsn1Error(
                    "Negative RELATIVE-OID arc %s at %s"
                    % (subOid, value)
                )
...
class RealEncoder(AbstractItemEncoder):
    ...
    def _chooseEncBase(self, value):
        ...
        for i in range(3):
            (sign, mantissa[i], encBase[i], exponent[i]) = (
                self._dropFloatingPoint(
    ...
    def encodeValue(self, value, asn1Spec, encodeFun, **options):
        ...
        if b == 10:
            ...
            return (
                b"\x03%dE%s%d" % (m, e == 0 and b"+" or b"", e),
                False,
                True,
            )
    ...
    def encodeValue(self, value, asn1Spec, encodeFun, **options):
        ...
        if asn1Spec is None:
            ...
            for idx, component in enumerate(value.values()):
                if namedTypes:
                    ...
                    if namedType.isOptional and not component.isValue:
                        if LOG:
                            LOG(
                                "not encoding OPTIONAL component %r"
                                % (namedType,)
                            )
                        continue

                    if (
                        namedType.isDefaulted
                        and component == namedType.asn1Object
                    ):
                        if LOG:
                            LOG(
                                "not encoding DEFAULT component %r"
                                % (namedType,)
                            )
                        continue
                    ...
                    if omitEmptyOptionals:
                        options.update(
                            ifNotEmpty=namedType.isOptional
                        )
                ...
                if namedTypes and namedType.openType:
                    ...
                    if wrapType.typeId in (
                        ...
                        substrate += encodeFun(
                            ...
                            **dict(
                                options,
                                wrapType=wrapType.componentType,
                            ),
                        )

                    else:
                        chunk = encodeFun(
                            component, asn1Spec, **options
                        )
                        ...
                        if wrapType.isSameTypeWith(component):
                            ...
                            substrate += encodeFun(
                                chunk, wrapType, **options
                            )

                            if LOG:
                                LOG(
                                    "wrapped with wrap type %r"
                                    % (wrapType,)
                                )
            ...
            for idx, namedType in enumerate(
                asn1Spec.componentType.namedTypes
            ):
                ...
                if (
                    ...
                    and namedType.name not in value
                ):
                    if LOG:
                        LOG(
                            "not encoding OPTIONAL component %r"
                ...
                if namedType.openType:
                    ...
                    if componentSpec.typeId in (
                        ...
                        chunk = encodeFun(
                            component, componentSpec, **options
                        )
...
class SequenceOfEncoder(AbstractItemEncoder):
    def _encodeComponents(
        ...
        for idx, component in enumerate(value):
            ...
            if wrapType is not None and not wrapType.isSameTypeWith(
                component
            ):
