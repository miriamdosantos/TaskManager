...
class TimeMixIn(object):
    ...
    def asDateTime(self):
        ...
        if text.endswith("Z"):
            ...
            if len(tz) != 4:
                raise error.PyAsn1Error(
                    "malformed time zone offset %s" % tz
                )
            ...
            try:
                ...
                raise error.PyAsn1Error(
                    "unknown time specification %s" % self
                )
        ...
        if (
            self._optionalMinutes
            and len(text) - self._yearsDigits == 6
        ):
            text += "0000"
        ...
        try:
            ...
            raise error.PyAsn1Error(
                "malformed datetime format %s" % self
            )
    ...
    def fromDateTime(cls, dt):
        ...
        """
        text = dt.strftime(
            cls._yearsDigits == 4 and "%Y%m%d%H%M%S" or "%y%m%d%H%M%S"
        )
