...
class Boolean(Integer):
    ...
    #: imposing constraints on |ASN.1| type initialization values.
    subtypeSpec = (
        Integer.subtypeSpec + constraint.SingleValueConstraint(0, 1)
    )
...
class BitString(base.SimpleAsn1Type):
    ...
    def __getitem__(self, i):
        if i.__class__ is slice:
            return self.clone(
                [self[x] for x in range(*i.indices(len(self)))]
            )
        else:
    ...
    def __add__(self, value):
        ...
        return self.clone(
            ...
            ).setBitLength(len(self._value) + len(value))
        )
    ...
    def __radd__(self, value):
        ...
        return self.clone(
            SizedInteger(
                value << len(self._value) | self._value
            ).setBitLength(len(self._value) + len(value))
        )
    ...
    def fromHexString(cls, value, internalFormat=False, prepend=None):
        ...
        """
        try:
            value = SizedInteger(value, 16).setBitLength(
                len(value) * 4
            )
    ...
    def fromBinaryString(
        ...
        """
        try:
            value = SizedInteger(value or "0", 2).setBitLength(
                len(value)
            )

        except ValueError as exc:
            raise error.PyAsn1Error(
                "%s.fromBinaryString() error: %s"
                % (cls.__name__, exc)
            )
            ...
            if not value:
                ...
                if value[-2:] == "'B":
                    return self.fromBinaryString(
                        value[1:-2], internalFormat=True
                    )
                elif value[-2:] == "'H":
                    return self.fromHexString(
                        value[1:-2], internalFormat=True
                    )
                else:
                    raise error.PyAsn1Error(
                ...
                try:
                    ...
                    bitPositions = [
                        self.namedValues[name] for name in names
                    ]
                ...
                return SizedInteger(number).setBitLength(
                    rightmostPosition + 1
                )

            elif value.startswith("0x"):
                return self.fromHexString(
                    value[2:], internalFormat=True
                )
                ...
                return self.fromBinaryString(
                    value, internalFormat=True
                )

        elif isinstance(value, (tuple, list)):
            return self.fromBinaryString(
                "".join([b and "1" or "0" for b in value]),
                internalFormat=True,
            )
...
class Real(base.SimpleAsn1Type):
    ...
    binEncBase = (
        None  # binEncBase = 16 is recommended for large numbers
    )
    ...
    def __getitem__(self, idx):
        if self._value in self._inf:
            raise error.PyAsn1Error(
                "Invalid infinite value operation"
            )
        else:
...
class SequenceOfAndSetOfBase(base.ConstructedAsn1Type):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if args:
            for key, value in zip(
                ("componentType", "tagSet", "subtypeSpec"), args
            ):
                if key in kwargs:
                    raise error.PyAsn1Error(
    ...
    def __len__(self):
        if (
            self._componentValues is noValue
            or not self._componentValues
        ):
            return 0
    ...
    def _cloneComponentValues(self, myClone, cloneValueFlag):
        for idx, componentValue in self._componentValues.items():
            if componentValue is not noValue:
                if isinstance(
                    componentValue, base.ConstructedAsn1Type
                ):
    ...
    def getComponentByPosition(
        ...
        if isinstance(idx, slice):
            ...
            return [
                self.getComponentByPosition(
                    subidx, default, instantiate
                )
    ...
    def setComponentByPosition(
        ...
        elif componentType is not None and (
            matchTags or matchConstraints
        ):
            subtypeChecker = (
    ...
    def prettyPrint(self, scope=0):
        ...
        for idx, componentValue in enumerate(self):
            ...
            if (
                componentValue is noValue
                and self.componentType is not None
            ):
    ...
    def prettyPrintType(self, scope=0):
        ...
        if self.componentType is not None:
            representation += " " * scope
            representation += self.componentType.prettyPrintType(
                scope
            )
        return representation + "\n" + " " * (scope - 1) + "}"
...
class SequenceAndSetBase(base.ConstructedAsn1Type):
    ...
    class DynamicNames(object):
        ...
        def __contains__(self, item):
            return (
                item in self._keyToIdxMap or item in self._idxToKeyMap
            )

        def __iter__(self):
            return (
                self._idxToKeyMap[idx]
                for idx in range(len(self._idxToKeyMap))
            )
    ...
    def __init__(self, **kwargs):
        ...
        if self._componentTypeLen:
            ...
            self._componentValues = noValue
        self._dynamicNames = (
            self._componentTypeLen or self.DynamicNames()
        )
    ...
    def values(self):
        for idx in range(
            self._componentTypeLen or len(self._dynamicNames)
        ):
            yield self[idx]
    ...
    def setComponentByPosition(
        ...
        if value is noValue:
            ...
            if componentTypeLen:
                subComponentType = componentType.getTypeByPosition(
                    idx
                )
                ...
                raise error.PyAsn1Error(
                    "%s undefined component type"
                    % componentType.__class__.__name__
                )

        elif (
            verifyConstraints or matchTags or matchConstraints
        ) and componentTypeLen:
    ...
    def isValue(self):
        ...
        if componentType:
            for idx, subComponentType in enumerate(
                componentType.namedTypes
            ):
                if (
                    subComponentType.isDefaulted
                    or subComponentType.isOptional
                ):
                    continue
    ...
    def prettyPrint(self, scope=0):
        ...
        for idx, componentValue in enumerate(self._componentValues):
            if (
                componentValue is not noValue
                and componentValue.isValue
            ):
                representation += " " * scope
                if self.componentType:
                    representation += (
                        self.componentType.getNameByPosition(idx)
                    )
                else:
                    representation += (
                        self._dynamicNames.getNameByPosition(idx)
                    )
                representation = "%s=%s\n" % (
                    representation,
    ...
    def prettyPrintType(self, scope=0):
        ...
        for idx, componentType in enumerate(
            ...
            if self.componentType:
                representation += (
                    '"%s"' % self.componentType.getNameByPosition(idx)
                )
            else:
                representation += (
                    '"%s"' % self._dynamicNames.getNameByPosition(idx)
                )
            representation = "%s = %s\n" % (
                representation,
...
class Choice(Set):
        ...
        try:
            ...
            if isinstance(component, base.ConstructedAsn1Type):
                myClone.setComponentByType(
                    tagSet,
                    component.clone(cloneValueFlag=cloneValueFlag),
                )
            else:
    ...
    def getName(self, innerFlag=False):
        ...
        if self._currentIdx is None:
            ...
            return self.componentType.getNameByPosition(
                self._currentIdx
            )
    ...
    def isValue(self):
        ...
        return (
            componentValue is not noValue and componentValue.isValue
        )
