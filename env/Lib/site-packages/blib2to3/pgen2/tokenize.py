...
def tokenize_loop(
    readline: Callable[[], str], tokeneater: TokenEater
) -> None:
...
def is_fstring_start(token: str) -> bool:
    return builtins.any(
        token.startswith(prefix) for prefix in fstring_prefix
    )
...
def generate_tokens(
    ...
    # `await` as keywords.
    async_keywords = (
        False if grammar is None else grammar.async_keywords
    )
    ...
    while 1:  # loop over lines in stream
            ...
            if endmatch:
                ...
                if fstring_state.current() in (
                    ...
                    if is_fstring_start(token):
                        fstring_start, token = (
                            _split_fstring_start_and_middle(token)
                        )
                        fstring_start_epos = (
                            spos[0],
        ...
        while pos < max:
            ...
            if (
                fstring_state.current() == STATE_IN_BRACES
                and parenlev == 0
            ):
