...
class FileProcessor:
        ...
        for token_type, text, start, end, line in self.tokens:
            ...
            if previous_row:
                ...
                if previous_row != start_row:
                    ...
                    previous_text = self.lines[row_index][
                        column_index
                    ]
                    if previous_text == "," or (
                        previous_text not in "{[("
                        and text not in "}])"
                    ):
                        text = f" {text}"
    ...
    def build_logical_line(self) -> tuple[str, str, _LogicalMapping]:
        ...
        comments, logical, mapping_list = (
            self.build_logical_line_tokens()
        )
        ...
        try:
            ...
            for tp, _, (s_line, _), (e_line, _), _ in file_tokens:
                ...
                if tp in (tokenize.NL, tokenize.NEWLINE):
                    ret.update(
                        self._noqa_line_range(min_line, max_line)
                    )
    ...
    def next_line(self) -> str:
        ...
        self.line_number += 1
        if (
            self.indent_char is None
            and line[:1] in defaults.WHITESPACE
        ):
    ...
    def should_ignore_file(self) -> bool:
        ...
        if not self.options.disable_noqa and any(
            ...
            defaults.NOQA_FILE.search(line) for line in self.lines
        ):
            LOG.warning(
