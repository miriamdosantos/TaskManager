...
def parse_plugin_options(
    ...
    *,
    ...
    paths_s = cfg.get(
        "flake8:local-plugins", "paths", fallback=""
    ).strip()
    ...
    return PluginOptions(
        ...
        enable_extensions=frozenset(
            _parse_option(
                cfg, "enable_extensions", enable_extensions
            ),
        ),
        require_plugins=frozenset(
...
def _flake8_plugins(
    ...
    pyflakes_meta = importlib.metadata.distribution(
        "pyflakes"
    ).metadata
    pycodestyle_meta = importlib.metadata.distribution(
        "pycodestyle"
    ).metadata
    ...
    for ep in eps:
        ...
        if ep.name == "F":
            yield Plugin(
                pyflakes_meta["name"], pyflakes_meta["version"], ep
            )
        elif ep.name in "EW":
            ...
            yield Plugin(
                pycodestyle_meta["name"],
                pycodestyle_meta["version"],
                ep,
            )
        else:
...
def _find_importlib_plugins() -> Generator[Plugin, None, None]:
    ...
    for dist in importlib.metadata.distributions():
        ...
        if meta["name"] in BANNED_PLUGINS:
            ...
            # pycodestyle
            yield from _flake8_plugins(
                eps, meta["name"], meta["version"]
            )
            continue
...
def _find_local_plugins(
    ...
    for plugin_type in ("extension", "report"):
        ...
        for plugin_s in utils.parse_comma_separated_list(
            cfg.get(
                "flake8:local-plugins", plugin_type, fallback=""
            ).strip(),
...
def _check_required_plugins(
    ...
    expected_names = {
        utils.normalize_pypi_name(name) for name in expected
    }
...
def _classify_plugins(
    ...
    for loaded in plugins:
        if (
            ...
            and loaded.plugin.entry_point.name
            not in opts.enable_extensions
        ):
