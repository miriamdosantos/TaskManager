...
class Application:
    ...
    def make_formatter(self) -> None:
        ...
        assert self.options is not None
        self.formatter = reporter.make(
            self.plugins.reporters, self.options
        )
    ...
    def report_benchmarks(self) -> None:
        ...
        for statistic in defaults.STATISTIC_NAMES + ("files",):
            ...
            per_second_description = (
                f"{statistic} processed per second"
            )
            add_statistic(
                (per_second_description, int(value / time_elapsed))
            )
