...
class Manager:
    ...
    def run_parallel(self) -> None:
        ...
        try:
            self.results = list(
                pool.imap_unordered(_mp_run, self.filenames)
            )
            pool.close()
            pool.join()
            pool_closed = True
    ...
    def run(self) -> None:
        ...
        try:
            ...
            raise exceptions.EarlyQuit(
                "Early quit while running checks"
            )
...
class FileChecker:
    ...
    def __init__(
        ...
        *,
        ...
        if self.processor is not None:
            ...
            self.should_process = (
                not self.processor.should_ignore_file()
            )
            self.statistics["physical lines"] = len(
                self.processor.lines
            )
    ...
    def report(
        ...
        self.results.append(
            (error_code, line_number, column, text, line)
        )
        return error_code

    def run_check(
        self, plugin: LoadedPlugin, **arguments: Any
    ) -> Any:
    ...
    @staticmethod
    def _extract_syntax_information(
        exception: Exception,
    ) -> tuple[int, int]:
        if (
    ...
    def run_logical_checks(self) -> None:
        ...
        comments, logical_line, mapping = (
            self.processor.build_logical_line()
        )
        if not mapping:
            return
        ...
        for plugin in self.plugins.logical_line:
            ...
            results = (
                self.run_check(plugin, logical_line=logical_line)
                or ()
            )
            for offset, text in results:
                line_number, column_offset = find_offset(
                    offset, mapping
                )
                if line_number == column_offset == 0:
                    LOG.warning(
                        "position of error out of bounds: %s", plugin
                    )
                self.report(
                    error_code=None,
                    line_number=line_number,
    ...
    def run_physical_checks(self, physical_line: str) -> None:
        ...
        for plugin in self.plugins.physical_line:
            ...
            result = self.run_check(
                plugin, physical_line=physical_line
            )
    ...
    def run_checks(self) -> tuple[str, Results, dict[str, int]]:
        ...
        try:
            ...
            self.report(
                code, row, column, f"{type(e).__name__}: {e.args[0]}"
            )
            return self.display_name, self.results, self.statistics
...
def _try_initialize_processpool(
    ...
    try:
        return multiprocessing.Pool(
            job_count, _mp_init, initargs=(argv,)
        )
