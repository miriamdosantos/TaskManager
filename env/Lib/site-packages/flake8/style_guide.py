...
class DecisionEngine:
    ...
    def make_decision(self, code: str) -> Decision:
        ...
        if isinstance(selected, Selected) and isinstance(
            ignored, Selected
        ):
            return Decision.Selected
        elif isinstance(selected, Ignored) and isinstance(
            ignored, Ignored
        ):
            ...
            selected is Ignored.Implicitly
            ...
            select = next(
                s for s in self.selected if code.startswith(s)
            )
            ignore = next(
                s for s in self.ignored if code.startswith(s)
            )
            ...
            raise AssertionError(
                f"unreachable {code} {selected} {ignored}"
            )
...
class StyleGuide:
    ...
    def handle_error(
        ...
        is_not_inline_ignored = (
            error.is_inline_ignored(disable_noqa) is False
        )
