...
class BaseFormatter:
    ...
    def show_statistics(self, statistics: Statistics) -> None:
        ...
        for error_code in statistics.error_codes():
            stats_for_error_code = statistics.statistics_for(
                error_code
            )
            ...
            self._write(
                f"{count:<5} {error_code} {statistic.message}"
            )

    def show_benchmarks(
        self, benchmarks: list[tuple[str, float]]
    ) -> None:
        ...
        for statistic, value in benchmarks:
            if isinstance(value, int):
                benchmark = int_format(
                    statistic=statistic, value=value
                )
            else:
                benchmark = float_format(
                    statistic=statistic, value=value
                )
    ...
    def show_source(self, error: Violation) -> str | None:
        ...
        """
        if (
            not self.options.show_source
            or error.physical_line is None
        ):
            return ""
