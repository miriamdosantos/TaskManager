...
def site_data_dir(
    ...
    if system == "win32":
        ...
        path = os.path.normpath(
            _get_win_folder("CSIDL_COMMON_APPDATA")
        )
        if appname:
            if appauthor is not False:
                path = os.path.join(path, appauthor, appname)
...
def user_cache_dir(
    ...
    if system == "win32":
        if appauthor is None:
            appauthor = appname
        path = os.path.normpath(
            _get_win_folder("CSIDL_LOCAL_APPDATA")
        )
        if appname:
            if appauthor is not False:
                path = os.path.join(path, appauthor, appname)
...
def user_log_dir(
    ...
    if system == "darwin":
        path = os.path.join(
            os.path.expanduser("~/Library/Logs"), appname
        )
    elif system == "win32":
...
def _get_win_folder_with_pywin32(csidl_name):
    ...
    dir = shell.SHGetFolderPath(
        0, getattr(shellcon, csidl_name), 0, 0
    )
