...
def last(iterable, default=_marker):
    ...
    try:
        ...
        elif hasattr(iterable, "__reversed__") and (
...
class peekable:
    ...
    def _get_slice(self, index):
        ...
        if step > 0:
            ...
            stop = (
                (-maxsize - 1) if (index.stop is None) else index.stop
            )
        else:
    ...
    def __getitem__(self, index):
        ...
        if index < 0:
            ...
            self._cache.extend(
                islice(self._it, index + 1 - cache_len)
            )
...
def one(iterable, too_short=None, too_long=None):
    ...
    try:
        ...
        raise (
            too_short
            or ValueError("too few items in iterable (expected 1)")
        ) from e
...
def interleave_longest(*iterables):
    ...
    i = chain.from_iterable(
        zip_longest(*iterables, fillvalue=_marker)
    )
...
def interleave_evenly(iterables, lengths=None):
    ...
    if lengths is None:
        ...
        raise ValueError(
            "Mismatching number of iterables and lengths."
        )
    ...
    delta_primary, deltas_secondary = (
        lengths_desc[0],
        lengths_desc[1:],
    )
...
def side_effect(
    func, iterable, chunk_size=None, before=None, after=None
):
...
def padded(iterable, fillvalue=None, n=None, next_multiple=False):
    ...
    if n is None:
        ...
        remaining = (
            (n - item_count) % n if next_multiple else n - item_count
        )
...
def distribute(n, iterable):
    ...
    return [
        islice(it, index, None, n)
        for index, it in enumerate(children)
    ]
...
def zip_offset(*iterables, offsets, longest=False, fillvalue=None):
    ...
    if len(iterables) != len(offsets):
        raise ValueError(
            "Number of iterables and offsets didn't match"
        )
...
def sort_together(iterables, key_list=(0,), key=None, reverse=False):
    ...
    return list(
        zip(
            *sorted(
                zip(*iterables), key=key_argument, reverse=reverse
            )
        )
    )
...
def unzip(iterable):
    ...
    *iterable* into memory. Note, however, that this function uses
    ...
    return tuple(
        map(itemgetter(i), it) for i, it in enumerate(iterables)
    )
...
def adjacent(predicate, iterable, distance=1):
    ...
    adjacent_to_selected = map(
        any, windowed(selected, 2 * distance + 1)
    )
    return zip(adjacent_to_selected, i2)


def groupby_transform(
    iterable, keyfunc=None, valuefunc=None, reducefunc=None
):
...
class numeric_range(abc.Sequence, abc.Hashable):
        ...
        if isinstance(other, numeric_range):
            ...
            if empty_self or empty_other:
                ...
                return (
                    ...
                    and self._get_by_index(-1)
                    == other._get_by_index(-1)
                )
        else:
            return False
    ...
    def __hash__(self):
        if self:
            return hash(
                (self._start, self._get_by_index(-1), self._step)
            )
        else:
    ...
    def _get_by_index(self, i):
        ...
        if i < 0 or i >= self._len:
            raise IndexError(
                "numeric range object index out of range"
            )
    ...
    def __getitem__(self, key):
        if isinstance(key, slice):
            return islice_extended(
                _islice_helper(self._iterable, key)
            )

        raise TypeError(
            "islice_extended.__getitem__ argument must be a slice"
        )
...
def _islice_helper(it, s):
    ...
    if s.step == 0:
        raise ValueError(
            "step argument must be a non-zero integer or None."
        )
    step = s.step or 1
...
class SequenceView(Sequence):
    ...
    def __repr__(self):
        return "{}({})".format(
            self.__class__.__name__, repr(self._target)
        )
        ...
        def outer_wrapper(f):
            def inner_wrapper(*args, **kwargs):
                ...
                return wrapping_func(
                    *wrapping_args_, **wrapping_kwargs
                )
    ...
    *pred* defaults to :func:`bool`, which will select truthy items:
    ...
    if window_size is None:
        try:
            ...
            return (
                len_iter - i - 1
                for i in locate(reversed(iterable), pred)
            )
        except TypeError:
            pass
...
def is_sorted(iterable, key=None, reverse=False, strict=False):
    ...
    compare = (
        (le if reverse else ge) if strict else (lt if reverse else gt)
    )
...
class callback_iter:
    ...
    * Provide *wait_seconds* to adjust how frequently the it is polled for
    ...
    def __init__(
        self, func, callback_kwd="callback", wait_seconds=0.1
    ):
        self._func = func
...
def minmax(iterable_or_value, *others, key=None, default=_marker):
    ...
    """
    iterable = (
        (iterable_or_value, *others) if others else iterable_or_value
    )
