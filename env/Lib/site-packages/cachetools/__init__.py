...
class FIFOCache(Cache):
    ...
    def __setitem__(
        self, key, value, cache_setitem=Cache.__setitem__
    ):
...
class MRUCache(Cache):
    ...
    def __init__(self, maxsize, getsizeof=None):
        ...
        warn(
            "MRUCache is deprecated", DeprecationWarning, stacklevel=2
        )
...
class TTLCache(_TimedCache):
    ...
    def __init__(
        self, maxsize, ttl, timer=time.monotonic, getsizeof=None
    ):
    ...
    def __setstate__(self, state):
        ...
        root.prev = root.next = root
        for link in sorted(
            self.__links.values(), key=lambda obj: obj.expires
        ):
            link.next = root
            link.prev = prev = root.prev
            prev.next = root.prev = link
...
class TLRUCache(_TimedCache):
    ...
    def __init__(
        self, maxsize, ttu, timer=time.monotonic, getsizeof=None
    ):
        _TimedCache.__init__(self, maxsize, timer, getsizeof)
    ...
    def expire(self, time=None):
        ...
        if len(order) > len(items) * 2:
            self.__order = order = [
                item for item in order if not item.removed
            ]
            heapq.heapify(order)
        expired = []
        cache_delitem = Cache.__delitem__
        cache_getitem = Cache.__getitem__
        while order and (
            order[0].removed or not (time < order[0].expires)
        ):
            item = heapq.heappop(order)
            if not item.removed:
                expired.append(
                    (item.key, cache_getitem(self, item.key))
                )
    ...
    def popitem(self):
        ...
        with self.timer as time:
            ...
            try:
                ...
                raise KeyError(
                    "%s is empty" % self.__class__.__name__
                ) from None
            else:
                return (key, self.pop(key))
        ...
        if info:
            ...
            if isinstance(cache, Cache):
                ...
                def getinfo():
                    ...
                    return _CacheInfo(
                        hits, misses, cache.maxsize, cache.currsize
                    )
