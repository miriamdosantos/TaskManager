        ...
        while i < len(s):
            if not in_fmt:
                ...
                conversion_flag_match = _must_match(
                    CONVERSION_FLAG_RE, s, i
                )
                conversion_flag = (
                    conversion_flag_match.group() or None
                )
                ...
                try:
                    ...
                    raise ValueError(
                        "end-of-string while parsing format"
                    )
                i += 1
...
def is_notimplemented_name_node(node):
    return (
        ...
        and getNodeName(node) == "NotImplemented"
    )
...
class Binding:
    ...
    def redefines(self, other):
        return (
            isinstance(other, Definition) and self.name == other.name
        )
...
class SubmoduleImportation(Importation):
    ...
    def __init__(self, name, source):
        ...
        assert "." in name and (
            not source or isinstance(source, ast.Import)
        )
...
class ExportBinding(Binding):
    ...
    def __init__(self, name, source, scope):
        ...
        # If concatenating lists or tuples
            ...
            while isinstance(
                currentValue.right, (ast.List, ast.Tuple)
            ):
...
class Scope(dict):
    ...
    def __repr__(self):
        ...
        return (
            f"<{scope_cls} at 0x{id(self):x} {dict.__repr__(self)}>"
        )
...
def _is_typing(node, typing_attr, scope_stack):
    ...
    context.
    """
    return _is_typing_helper(
        node, lambda x: x == typing_attr, scope_stack
    )
...
class Checker:
    ...
    def __init__(
        ...
        try:
            ...
            raise RuntimeError(
                "No scope implemented for the node %r" % tree
            )
        ...
        for scope in self.deadScopes:
            ...
            if isinstance(scope, FunctionScope):
                for name, binding in scope.unused_assignments():
                    self.report(
                        messages.UnusedVariable, binding.source, name
                    )
            ...
            if all_binding and not isinstance(
                all_binding, ExportBinding
            ):
                all_binding = None
            ...
            for value in scope.values():
                if isinstance(value, Importation):
                    ...
                    for node in value.redefined:
                        ...
                        self.report(
                            messg, node, value.name, value.source
                        )

    def report(self, messageClass, *args, **kwargs):
        self.messages.append(
            messageClass(self.filename, *args, **kwargs)
        )
    ...
    def getCommonAncestor(self, lnode, rnode, stop):
        ...
        if lnode._pyflakes_depth > rnode._pyflakes_depth:
            return self.getCommonAncestor(
                lnode._pyflakes_parent, rnode, stop
            )
        if lnode._pyflakes_depth < rnode._pyflakes_depth:
            return self.getCommonAncestor(
                lnode, rnode._pyflakes_parent, stop
            )
        return self.getCommonAncestor(
    ...
    def getScopeNode(self, node):
        return self._getAncestor(
            node, tuple(Checker._ast_node_scope.keys())
        )
    ...
    def addBinding(self, node, value):
        ...
        if (
            ...
            if isinstance(existing, Importation) and isinstance(
                ...
                if (
                    ...
                    and (
                        value.name != "_"
                        or isinstance(existing, Importation)
                    )
                    and not is_typing_overload(
                        existing, self.scopeStack
                    )
                ):
                    self.report(
        ...
        # in scope
        if value.name not in self.scope or not isinstance(
            value, Annotation
        ):
            cur_scope_pos = -1
            # As per PEP 572, use scope in which outermost generator is defined
            while isinstance(
                value, NamedExprAssignment
            ) and isinstance(
    ...
    def handleNodeLoad(self, node, parent):
        ...
        if importStarred:
            ...
            from_list = ", ".join(sorted(from_list))
            self.report(
                messages.ImportStarUsage, node, name, from_list
            )
            return
    ...
    def handleNodeStore(self, node):
        ...
        if (
            ...
            for scope in self.scopeStack[:-1]:
                if not isinstance(
                    scope, (FunctionScope, ModuleScope)
                ):
                    continue
        ...
        if (
            ...
            parent_stmt, (FOR_TYPES, ast.comprehension)
        ) or (
            ...
            binding = ExportBinding(
                name, node._pyflakes_parent, self.scope
            )
    ...
    def handleNodeDelete(self, node):
        ...
        def on_conditional_branch():
            ...
            while current:
                if isinstance(
                    current, (ast.If, ast.While, ast.IfExp)
                ):
                    return True
    ...
    def handleDoctests(self, node):
        try:
            ...
            examples = docstring and self._getDoctestExamples(
                docstring
            )
        ...
        with self.in_scope(DoctestScope):
            ...
            for example in examples:
                try:
                    ...
                    self.report(
                        messages.DoctestSyntaxError, node, position
                    )
                else:
                    self.offset = (
    ...
    def handleStringAnnotation(
        self, s, node, ref_lineno, ref_col_offset, err
    ):
        try:
            tree = ast.parse(s)
        except SyntaxError:
    ...
    def _handle_string_dot_format(self, node):
        try:
            placeholders = tuple(
                parse_format_string(node.func.value.value)
            )
        except ValueError as e:
            self.report(
                messages.StringDotFormatInvalidFormat, node, e
            )
            return
        ...
        def _add_key(fmtkey):
            ...
            try:
                ...
                if auto is True:
                    self.report(
                        messages.StringDotFormatMixingAutomatic, node
                    )
                    return True
                else:
                    auto = False
        ...
        for _, fmtkey, spec, _ in placeholders:
            ...
            if spec is not None:
                try:
                    spec_placeholders = tuple(
                        parse_format_string(spec)
                    )
                except ValueError as e:
                    self.report(
                        messages.StringDotFormatInvalidFormat, node, e
        ...
        extra_positional = (
            substitution_positional - placeholder_positional
        )
    ...
    def CALL(self, node):
        ...
        if (
            ...
            omit += ["keywords"]
            annotated += [
                k.value for k in node.keywords if k.arg == "bound"
            ]
            not_annotated += [
            ...
            if (
                ...
                and all(
                    isinstance(x, (ast.Tuple, ast.List))
                    and len(x.elts) == 2
                ...
                omit += ["args"]
                annotated += [
                    elt.elts[1] for elt in node.args[1].elts
                ]
                not_annotated += [
                ...
                not_annotated += [
                    (elt, "elts") for elt in node.args[1].elts
                ]
    ...
    def _handle_percent_format(self, node):
        ...
        if (
            ...
            if positional and positional_count != substitution_count:
                ...
                self.report(
                    messages.PercentFormatExpectedMapping, node
                )
        ...
        if isinstance(node.right, ast.Dict) and all(
            ...
            if positional and positional_count > 1:
                self.report(
                    messages.PercentFormatExpectedSequence, node
                )
                return
    ...
    AND = OR = ADD = SUB = MULT = DIV = MOD = POW = LSHIFT = (
        RSHIFT
    ) = BITOR = BITXOR = BITAND = FLOORDIV = INVERT = NOT = UADD = (
        USUB
    ...
    def DICT(self, node):
        ...
        for key in duplicate_keys:
            key_indices = [
                i for i, i_key in enumerate(keys) if i_key == key
            ]
    ...
    def ARG(self, node):
        self.addBinding(
            node, Argument(node.arg, self.getScopeNode(node))
        )
        ...
        with self._type_param_scope(node):
            ...
            with self.in_scope(ClassScope):
                ...
                if (
                    ...
                    self.deferFunction(
                        lambda: self.handleDoctests(node)
                    )
        ...
        if isinstance(node.ctx, ast.Store):
            ...
            for i, n in enumerate(node.elts):
                if isinstance(n, ast.Starred):
                    if has_starred:
                        self.report(
                            messages.TwoStarredExpressions, node
                        )
            ...
            if (
                star_loc >= 1 << 8
                or len(node.elts) - star_loc - 1 >= 1 << 24
            ):
                self.report(
    ...
    def IMPORTFROM(self, node):
        ...
        for alias in node.names:
            name = alias.asname or alias.name
            if node.module == "__future__":
                importation = FutureImportation(
                    name, node, self.scope
                )
                ...
                if not isinstance(self.scope, ModuleScope):
                    self.report(
                        messages.ImportStarNotPermitted, node, module
                    )
                    continue
    ...
    def ANNASSIGN(self, node):
        ...
        if node.value:
            ...
            if _is_typing(
                node.annotation, "TypeAlias", self.scopeStack
            ):
    ...
    MATCH = MATCH_CASE = MATCHCLASS = MATCHOR = MATCHSEQUENCE = (
        handleChildren
    )
