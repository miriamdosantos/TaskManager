...
class LoggingReporter:
    ...
    def syntaxError(self, filename, msg, lineno, offset, line):
        self.log.append(
            ("syntaxError", filename, msg, lineno, offset, line)
        )
...
class TestIterSourceCode(TestCase):
    ...
    def test_singleFile(self):
        ...
        self.assertEqual(
            list(iterSourceCode([self.tempdir])), [childpath]
        )
...
class TestReporter(TestCase):
    ...
    def test_syntaxError(self):
        ...
        reporter.syntaxError(
            "foo.py", "a problem", 3, 8, "bad line of source"
        )
        self.assertEqual(
            (
        ...
        with self.makeTempFile("import foo") as sourcePath:
            ...
            self.assertEqual(
                ...
                [
                    (
                        "flake",
                        str(UnusedImport(sourcePath, Node(1), "foo")),
                    )
                ],
            )
            ...
            self.assertEqual(
                ...
                sorted(
                    [
                        (
                            "flake",
                            str(UnusedImport(file1, Node(1), "baz")),
                        ),
                        (
                            "flake",
                            str(
                                UnusedImport(
                                    file2, Node(1), "contraband"
                                )
                            ),
                        ),
                    ]
...
class IntegrationTests(TestCase):
    ...
    def test_fileWithFlakes(self):
        ...
        with open(self.tempfilepath, "wb") as fd:
            fd.write(b"import contraband\n")
        d = self.runPyflakes([self.tempfilepath])
        expected = UnusedImport(
            self.tempfilepath, Node(1), "contraband"
        )
