...
def font_size_splitter(font_map):
    ...
    for font in fonts:
        ...
        if length <= FONT_SMALL_THRESHOLD:
            ...
            length > FONT_SMALL_THRESHOLD
            and length <= FONT_MEDIUM_THRESHOLD
        ):
            medium_font.append(font)
        elif (
            length > FONT_MEDIUM_THRESHOLD
            and length <= FONT_LARGE_THRESHOLD
        ):
    ...
    try:
        if artname == "UnicodeEncodeError":
            raise UnicodeEncodeError(
                "test", "", 42, 43, "test unicode-encode-error"
            )
...
def art(artname, number=1):
    ...
    if artname in ["random", "rand", "rnd"]:
        ...
        distance_list = list(
            map(lambda x: distance_calc(artname, x), arts)
        )
...
def wizard_font(text):
    ...
    if text_length <= TEXT_XLARGE_THRESHOLD:
        ...
        text_length > TEXT_XLARGE_THRESHOLD
        and text_length <= TEXT_LARGE_THRESHOLD
    ):
        font = random.choice(LARGE_WIZARD_FONT)
    elif (
        text_length > TEXT_LARGE_THRESHOLD
        and text_length <= TEXT_MEDIUM_THRESHOLD
    ):
...
def indirect_font(font, text):
    ...
    if font not in fonts:
        distance_list = list(
            map(lambda x: distance_calc(font, x), fonts)
        )
...
def __word2art(word, font, chr_ignore, letters, next_word):
    ...
    for i in range(len(split_list[0])):
        ...
        for j in range(len(split_list)):
            if (
                j > 0
                and (i == 1 or i == len(split_list[0]) - 2)
                and font == "block"
            ):
                temp = temp + " "
...
def text2art(
    text, font=DEFAULT_FONT, chr_ignore=True, decoration=None
):
    r"""
...
def set_default(
    ...
    if isinstance(font, str) is False:
        raise artError(FONT_TYPE_ERROR)
    if (
        isinstance(decoration, str) is False
