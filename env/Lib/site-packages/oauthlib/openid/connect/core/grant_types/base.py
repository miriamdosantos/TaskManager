...
class GrantTypeBase:
    ...
    def validate_authorization_request(self, request):
        ...
        """
        return self.proxy_target.validate_authorization_request(
            request
        )
    ...
    def id_token_hash(self, value, hashfunc=hashlib.sha256):
        ...
        return (
            base64.urlsafe_b64encode(digest[:left_most])
            .decode()
            .rstrip("=")
        )
    ...
    def add_id_token(self, token, token_handler, request, nonce=None):
        ...
        if (
            ...
            and "id_token" not in request.response_type
        ):
            return token
        ...
        if "access_token" in token:
            id_token["at_hash"] = self.id_token_hash(
                token["access_token"]
            )
    ...
    def openid_authorization_validator(self, request):
        ...
        if "none" in prompt:
            ...
            if len(prompt) > 1:
                ...
                raise InvalidRequestError(
                    request=request, description=msg
                )

            if not self.request_validator.validate_silent_login(
                request
            ):
