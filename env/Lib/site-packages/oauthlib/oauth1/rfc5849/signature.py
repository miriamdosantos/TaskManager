...
def collect_parameters(
    ...
    if exclude_oauth_signature:
        unescaped_params = list(
            filter(
                lambda i: i[0] != "oauth_signature", unescaped_params
            )
        )
...
def normalize_parameters(params) -> str:
    ...
    # .. _`Section 3.6`: https://tools.ietf.org/html/rfc5849#section-3.6
    key_values = [
        (utils.escape(k), utils.escape(v)) for k, v in params
    ]
...
def _sign_hmac(
    ...
    **HMAC-SHA256**
    ...
    return binascii.b2a_base64(signature.digest())[:-1].decode(
        "utf-8"
    )
...
def _verify_hmac(
    ...
    if not match:
        log.debug(
            "Verify HMAC failed: signature base string: %s",
            sig_base_str,
        )
    return match
...
def verify_hmac_sha1(
    ...
    return _verify_hmac(
        "SHA-1", request, client_secret, resource_owner_secret
    )
...
def sign_hmac_sha256(
    base_string, client_secret, resource_owner_secret
):
    """
...
def _verify_rsa(
    hash_algorithm_name: str, request, rsa_public_key: str
):
    """
