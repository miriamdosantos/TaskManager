...
SIGNATURE_HMAC = (
    SIGNATURE_HMAC_SHA1  # deprecated variable for HMAC-SHA1
)
...
class Client:
    ...
    def __repr__(self):
        ...
        attrs["client_secret"] = (
            "****" if attrs["client_secret"] else None
        )
        ...
        return "<{} {}>".format(
            self.__class__.__name__, attribute_str
        )
    ...
    def get_oauth_signature(self, request):
        ...
        normalized_params = signature.normalize_parameters(
            collected_params
        )
        ...
        log.debug(
            "Signing: signature base string: {}".format(base_string)
        )

        if self.signature_method not in self.SIGNATURE_METHODS:
            raise ValueError("Invalid signature method.")

        sig = self.SIGNATURE_METHODS[self.signature_method](
            base_string, self
        )
    ...
    def get_oauth_params(self, request):
        ...
        content_type_eligible = (
            content_type
            and content_type.find("application/x-www-form-urlencoded")
            < 0
        )
    ...
    def sign(
        ...
        # sanity check
        content_type = request.headers.get("Content-Type", None)
        multipart = content_type and content_type.startswith(
            "multipart/"
        )
        should_have_params = (
        ...
        if self.decoding:
            log.debug(
                "Encoding URI, headers and body to %s.", self.decoding
            )
            uri = uri.encode(self.decoding)
            body = body.encode(self.decoding) if body else body
            new_headers = {}
            for k, v in headers.items():
                new_headers[k.encode(self.decoding)] = v.encode(
                    self.decoding
                )
            headers = new_headers
        return uri, headers, body
