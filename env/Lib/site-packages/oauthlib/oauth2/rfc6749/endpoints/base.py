...
class BaseEndpoint:
    ...
    def valid_request_methods(self, valid_request_methods):
        if valid_request_methods is not None:
            valid_request_methods = [
                x.upper() for x in valid_request_methods
            ]
    ...
    def _raise_on_unsupported_token(self, request):
        ...
        if (
            ...
            and request.token_type_hint
            not in self.supported_token_types
        ):
    ...
    def _raise_on_bad_method(self, request):
        ...
        if (
            ...
            not in self.valid_request_methods
        ):
            raise InvalidRequestError(
                request=request,
                description=(
...
def catch_errors_and_unavailability(f):
    ...
    def wrapper(endpoint, uri, *args, **kwargs):
        if not endpoint.available:
            ...
            log.info(
                "Endpoint unavailable, ignoring request %s." % uri
            )
            return {}, e.json, 503
        ...
        if endpoint.catch_errors:
            try:
                ...
                error = ServerError()
                log.warning(
                    "Exception caught while processing request, %s."
                    % e
                )
