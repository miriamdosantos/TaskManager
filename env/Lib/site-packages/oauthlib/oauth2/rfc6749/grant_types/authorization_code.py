...
class AuthorizationCodeGrant(GrantTypeBase):
    ...
    def create_authorization_response(self, request, token_handler):
        ...
        # https://tools.ietf.org/html/rfc6749#appendix-B
            ...
            request.redirect_uri = (
                request.redirect_uri or self.error_uri
            )
    ...
    def validate_authorization_request(self, request):
        ...
        if (
            self.request_validator.is_pkce_required(
                request.client_id, request
            )
            is True
        ):
            if request.code_challenge is None:
                raise errors.MissingCodeChallengeError(
                    request=request
                )
        ...
        if request.code_challenge is not None:
            ...
            request_info["code_challenge_method"] = (
                request.code_challenge_method
            )
    ...
    def validate_token_request(self, request):
        ...
        if self.request_validator.client_authentication_required(
            ...
            if not self.request_validator.authenticate_client(
                ...
                log.debug(
                    "Client authentication failed, %r.", request
                )
        ...
        request.client_id = (
            request.client_id or request.client.client_id
        )
        ...
        if challenge is not None:
            ...
            if challenge_method is None:
                raise errors.InvalidGrantError(
                    request=request,
                    description="Challenge method not found",
                )
        ...
        for attr in ("user", "scopes"):
            if getattr(request, attr, None) is None:
                log.debug(
                    "request.%s was not set on code validation.", attr
                )
        ...
        if request.redirect_uri is None:
            ...
            log.debug(
                "Using default redirect_uri %s.", request.redirect_uri
            )
            ...
            log.debug(
                "Using provided redirect_uri %s", request.redirect_uri
            )
    ...
    def validate_code_challenge(
        self, challenge, challenge_method, verifier
    ):
