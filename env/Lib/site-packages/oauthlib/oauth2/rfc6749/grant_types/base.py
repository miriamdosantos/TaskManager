...
class GrantTypeBase:
    ...
    def __init__(self, request_validator=None, **kwargs):
        self.request_validator = (
            request_validator or RequestValidator()
        )
    ...
    def _create_cors_headers(self, request):
        ...
        if not is_secure_transport(origin):
            log.debug(
                'Origin "%s" is not HTTPS, CORS not allowed.', origin
            )
            return {}
            ...
            log.debug(
                'Invalid origin "%s", CORS not allowed.', origin
            )
            return {}
        else:
            log.debug(
                'Valid origin "%s", injecting CORS headers.', origin
            )
