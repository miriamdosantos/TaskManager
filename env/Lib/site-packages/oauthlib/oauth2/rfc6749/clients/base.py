...
class Client:
    ...
    def add_token(
        ...
        **kwargs
        ...
        token_placement = (
            token_placement or self.default_token_placement
        )
        ...
        if (
            not self.token_type.lower()
            in case_insensitive_token_types
        ):
            raise ValueError(
                "Unsupported token type: %s" % self.token_type
            )
    ...
    def _add_bearer_token(
        ...
        if token_placement == AUTH_HEADER:
            headers = tokens.prepare_bearer_headers(
                self.access_token, headers
            )
    ...
    def create_code_verifier(self, length):
        ...
        if not length >= 43:
            raise ValueError(
                "Length must be greater than or equal to 43"
            )

        if not length <= 128:
            raise ValueError(
                "Length must be less than or equal to 128"
            )
    ...
    def create_code_challenge(
        self, code_verifier, code_challenge_method=None
    ):
        ...
        if code_challenge_method == "S256":
            ...
            sha256_val = h.digest()
            code_challenge = bytes.decode(
                base64.urlsafe_b64encode(sha256_val)
            )
