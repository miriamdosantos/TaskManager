...
IPvFuture = (
    r"v %(HEXDIG)s+ \. (?: %(unreserved)s | %(sub_delims)s | : )+"
    % locals()
)
...
host = (
    r"(?: %(IP_literal)s | %(IPv4address)s | %(reg_name)s )"
    % locals()
)
...
authority = (
    r"(?: %(userinfo)s @)? %(host)s (?: : %(port)s)?" % locals()
)
...
path_absolute = (
    r"/ (?: %(segment_nz)s (?: / %(segment)s )* )?" % locals()
)
...
relative_ref = (
    r"%(relative_part)s (?: \? %(query)s)? (?: \# %(fragment)s)?"
    % locals()
)
...
absolute_URI = (
    r"^(?: %(scheme)s : %(hier_part)s (?: \? %(query)s )? )$"
    % locals()
)
