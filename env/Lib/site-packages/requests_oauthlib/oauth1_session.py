...
class OAuth1Session(requests.Session):
    ...
    def fetch_access_token(
        self, url, verifier=None, **request_kwargs
    ):
        ...
        token = self._fetch_token(url, **request_kwargs)
        log.debug(
            "Resetting verifier attribute, should not be used anymore."
        )
    ...
    def _populate_attributes(self, token):
        if "oauth_token" in token:
            self._client.client.resource_owner_key = token[
                "oauth_token"
            ]
        else:
            raise TokenMissing(
                "Response does not contain a token: {resp}".format(
                    resp=token
                ),
                token,
            )
    ...
    def _fetch_token(self, url, **request_kwargs):
        log.debug(
            "Fetching token from %s using client %s",
            url,
            self._client.client,
        )
        ...
        log.debug(
            "Updating internal client attributes from token data."
        )
        self._populate_attributes(token)
        self.token = token
        return token
