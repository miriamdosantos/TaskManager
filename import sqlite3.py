...
def register_user(cursor):
    while True:
        try:
            username = input('Enter username (at least 5 characters long and contain only alphanumeric characters.):')
            password = input('Enter password (at least 8 characters long.): ')

            if len(username) < 5 or not username.isalnum():
                raise ValueError("Username must be at least 5 characters long and contain only alphanumeric characters.")
            if len(password) < 8:
...
def login_user(cursor):
    while True:
        ...
        if user:
            ...
            quit_input = input('Invalid username or password. Press Enter to try again, (or type "quit" to exit):').lower()
...
def validate_date(due_date_str):
    try:
        ...
        raise ValueError(f'Error: {e}. Please enter the date in the format DD-MM-YYYY.')
...
def add_task(username, cursor, conn):
    while True:
        name = validate_name(input("Enter the task name (or type 'quit' to exit): "))
        if not name:
            break
        
        due_date = validate_date(input("Enter the due date (DD-MM-YYYY) (or type 'quit' to exit): "))
        if not due_date:
            break
        
        priority = validate_priority(input('Enter the priority (low, medium, high) (or type "quit" to exit): ').lower())
        if not priority:
            break
        
        category = validate_category(input('Enter the category: P - (Personal), B - (Business) (or type "quit" to exit): ').upper())
        if not category:
            break
        
        description = validate_description(input('Enter the description: (maximum 50 characters, press Enter to skip) (or type "quit" to exit): '))
        if description is None:
            break
        
        status_abbreviation = validate_status(input('Enter the task status : C - (Complete); P - (Pending); IP - (In Progress) (or type "quit" to exit): ').upper())
        ...
        try:
            ...
            cursor.execute("INSERT INTO tasks (user_id, name, due_date, priority, category, description, status) VALUES (?, ?, ?, ?, ?, ?, ?)",
                           (user_id, name, due_date, priority, category, description, status))
...
def list_tasks(cursor, username, category_input):
    ...
    cursor.execute("SELECT id, name FROM tasks WHERE user_id = ? AND category = ?", (user_id, category))
...
def view_all_list(cursor, username):
    ...
    cursor.execute("SELECT id, name, category FROM tasks WHERE user_id = ?", (user_id,))
    tasks = cursor.fetchall()
    ...
    for task_id, task_name, task_category in tasks:
        print(f"Task ID: {task_id}, Name: {task_name}, Category: {task_category.capitalize()}")
...
def sort_by_due_date(cursor, username):
    ...
    cursor.execute("SELECT id, name, due_date FROM tasks WHERE user_id = ? ORDER BY due_date", (user_id,))
    tasks = cursor.fetchall()
...
def sort_by_priority(cursor, username):
    ...
    cursor.execute("SELECT id, name, priority FROM tasks WHERE user_id = ? ORDER BY priority", (user_id,))
    tasks = cursor.fetchall()
...
def sort_by_status(cursor, username):
    ...
    cursor.execute("SELECT id, name, status FROM tasks WHERE user_id = ? ORDER BY status", (user_id,))
    tasks = cursor.fetchall()
